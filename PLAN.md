...
- 2026-09-23: `verdict` VALUES may differ for sparse rows — a BEHAVIOUR change, **not** a schema change. Same field, same three strings (`Clean`/`Promising`/`Caution`), same position, so no consumer needs a code change. `_compute_verdict` now additionally requires a minimum number of enrichment SOURCES that actually returned a value (`verdict_thresholds.clean_min_real_signals` = 3, `promising_min_real_signals` = 2). Why: scoring excludes null signals from the weighted average and renormalises the rest, so a domain we know LESS about could outscore one we know more about — on 2026-09-22 the highest-scoring row on the live site (87, the only `Clean` ever published) earned it because `open_page_rank` and `cert_history` were both missing, while every fully-enriched row topped out at 61. The floor is an ADDITIONAL necessary condition and can only demote, never promote; the score maths is deliberately untouched. Measured impact on the 216 live rows: exactly one verdict changes.
  Consumers that recompute a verdict from `score` alone (the frontend/newsletter fallback path used for sample data and pre-2026-05-17 payloads) do NOT inherit this floor and would still paint a sparse high scorer as Clean — read the `verdict` field, do not recompute it.
  Same date, related: the publish completeness gate counts SOURCES not FIELDS (`publish_completeness_sources`). Not a payload change at all, but it decides which rows exist: `previous_registrar` was impossible for an AVAILABLE domain (0 of 216 rows) and the two Wayback fields were one source counted twice, which is how one third-party API migration produced two days of zero published domains on 2026-09-21/22.
