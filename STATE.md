...
Last updated: 2026-05-10 (First Verisign-enabled cron OOM-killed at 6m54s during the .com zone parse — Python `set[str]` of ~160M apex names exceeded GHA's 16 GB runner RAM ceiling (peak ~15.5 GB just for today_set; day-2 yesterday_set diff would push to ~30 GB). Investigation confirmed pure-scale wall: no concurrency leak, no architectural bug — `zone_parser.parse_zone` is linear in apex count and .com is ~12.5× larger than our previous biggest (.org at 12.8M). Reverted .com to `tlds.pending` while OVH KS-6 dedicated-server migration is provisioned (ordered today). `.net` stays in `tlds.approved` — comparable scale to .org, parses fine. Tomorrow's cron is `.net`'s first production run. `.com` returns to approved once OVH migration validates pipeline runs cleanly at .com scale. 336/336 tests passing, config-only revert.)
