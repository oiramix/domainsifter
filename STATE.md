...
**5. Frontend — methodology section.** `src/components/Methodology.astro` gains a new Step 5 card with the `link-2` icon (two interlocking links, Lucide stroke geometry) and the exact body Mario specified: *"We cross-reference each candidate with Common Crawl's monthly domain webgraph — the most recent 3 months of the live web. The result: how many distinct source domains currently link to this domain, not historical claims that may no longer hold."* The deliberate positioning copy — honest about CC's 3-month window, implicit contrast with historical-cumulative claims from other tools — lands without naming names. Grid expands from 6 cards to 7 (one empty cell at lg:grid-cols-3 in the bottom row; acceptable for methodology accuracy over visual neatness).
