...
1. **Common Crawl is 4 months stale and was NEVER automated.** Release `cc-main-2026-feb-mar-apr`, pulled 2026-05-13: 18.16 GB edges + 0.92 GB vertices + 6.6 GB derived SQLite in R2. `cc_refresh.py` has **no systemd timer and no cron entry** — it has only ever been run by hand, once. `cc_source_domain_count` carries scoring weight **0.30, equal to wayback_snapshots**, so this is a material input running on stale data. CC publishes the domain-level hyperlink graph ~3x/year. Needs: check for a newer release, run the refresh on the box (25 GB, hours), update `cc_backlinks.latest_release`, add a timer, decide cadence. **Start a fresh session for this.**
...
7. **Newsletter intro text names the CC release** as of `7755c61`, so it must be updated whenever `latest_release` changes. Today's already-generated draft still contains the old "last 3 months" wording.
