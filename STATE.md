...
Last updated: 2026-09-23 — **Every pipeline step now declares whether it actually did anything (2026-09-23).** Every incident here shares one shape: a step fails soft, returns empty, the next step treats empty as legitimate, and nobody learns until output drops — LLM credits 8 WEEKS, Common Crawl 4 MONTHS, OpenPageRank 2 DAYS, crt.sh ongoing. Fail-soft was implemented as fail-SILENT. Three changes: (1) `pipeline.py` emits a per-source `Enrichment coverage (N candidates): open_page_rank=0/25 ...` line and `send_report.py` escalates the subject to **🚨 SOURCE DEAD (open_page_rank)** when an alarm source returns nothing — naming the source IS the one-minute fix, and a missing line is itself reported because silence is the failure mode. (2) The publish completeness gate counts SOURCES not FIELDS: `previous_registrar` is impossible for an AVAILABLE domain (0/216 forever) and the two Wayback fields were ONE source counted twice, letting Wayback carry 40% of the gate — together the reason one API migration caused two days of zero output. Now 2 of 3; live rows 184 at 1.00, 32 at 0.67, none below the gate. Deliberately NOT outage-proof: lose OPR again and publishing correctly stops, but change 1 makes it visible that morning. (3) Missing data can no longer buy a verdict — Clean needs 3 real sources, Promising 2. The score maths is UNTOUCHED by decision; the floor only demotes. Exactly ONE verdict changes across 216 live rows: electricianyorkpa.com 87 Clean → Caution, the row that was top of the site only because two signals were missing while every fully-enriched row topped out at 61. Also today: the OPR fix is confirmed — completeness rejections 12 → 12 → 0 and 14 new domains published. **1967 tests passing.** Open: the electricianyorkpa.com archive page still asserts the retracted Clean verdict and reasons from absence; crt.sh is the next weak source (~12 failures today, one at 65s) and is now visible rather than invisible. Full detail in the "2026-09-23" section at the end of this file. Earlier (2026-09-22) — **Two days of zero new domains: OpenPageRank migrated and it was never the filters (2026-09-22).** The funnel was healthy — 177,290 drops → 25 confirmed available → 17 past the spam filter — and then ALL 17 died at the publish quality floor on **enrichment completeness**. Cause: **OpenPageRank moved from DomCop to Keywords Everywhere and the old API was switched off**; `openpagerank.com` 301s to `openpagerank.keywordseverywhere.com`, which 404s the old path, and because `requests` follows redirects the log showed a bare 404 against a URL nobody configured — it read like an outage, not a migration. The gate needs 3 of 5 fields; candidates had 2/5 = 0.40 vs a 0.50 threshold. **With OPR alive it would have been 3/5 and all 25 would have published.** Timeline is unambiguous: 0 completeness rejections on 09-18/19/20 (45, 79, 40 new published), then 12 on 09-21 and 12 on 09-22 — yesterday's zero had the same cause, masked by the token-outage headline. **Structural fragility exposed**: `previous_registrar` is present in 0 of 216 published rows (impossible for an AVAILABLE domain, the only kind we publish) and `cert_history` depends on crt.sh which 502s most days — so the gate scores 4 obtainable fields against a 5-field denominator and **OPR was silently load-bearing for the entire publish step**. Gate left unchanged by the owner's decision; revisit when OPR is healthy. Fixed: rewrote the client for POST `/v1/domains/bulk` + `Authorization: Bearer`, new `opr_live_` key minted and written to `.env` (legacy DomCop keys are rejected 401, confirmed live; KE and OPR keys are DISTINCT credentials). `found:false` returns `0.0`, NOT `{}` — load-bearing, since an absent field costs a completeness point and would reject exactly the obscure domains we exist to surface. **Second defect caught before declaring victory**: the new API is far slower (min 2.56s / median 6.57s / max 29.91s; 3 of 10 over the global 10s timeout), so ~a third of calls would fail and the breaker would take OPR dark mid-run — the pipeline would have LOOKED fixed and still published nothing. Added per-source `api_timeout_seconds` (OPR 45s). Verified: **12/12 of today's rejected candidates now return a score**, 2/5 → 3/5. Batching (100 domains/request) deferred — it needs hard rule 8 revisited. **1870 tests passing.** Two days of finds are lost (17 + 25), not recoverable via carryover since they never published. **Lesson: the daily report shows enrichment completeness nowhere** — a line reading `open_page_rank: 0/25` on 09-21 would have cost one minute instead of two days. Full detail in the "2026-09-22" section at the end of this file. Earlier (2026-09-21) — **Token outage published 247 unscreened domains; list cleaned and the classifier's instability fixed at the root (2026-09-21).** The Max subscription ran out of tokens on another project, so the 09:00 UTC run had no LLM window and published **247 domains with NO toxic screening**. **The alarm worked** — the email subject read `🚨 TOXIC SCREEN OFF / 🚨 PHASE 2 FALLBACK`, which is the 2026-09-18 escalation design doing its job; the same failure went unnoticed for ~8 weeks after the July outage. Cleanup: three sweeps plus hand verification removed **17 toxic domains** (denylist 6 → 23) and **5 archive pages**, taking screening coverage from **76/247 (31%) to 137/230 (60%)**. Worst finds: `walgreenslistens.site` (Walgreens-survey scam), `martyflynnforstatesenate.com` (Indonesian slots on a squatted campaign domain), `attorneysportorange.com` (`9001cc金沙` casino SEO cloaked as a machinery firm — **the LLM called it `legitimate`**). **Root cause of the verdict instability found**: a dry run found 9 toxic, the real run 20 min later found 8 with only 5 in common, because `snapshot_classifier._fetch_excerpt_for` re-fetched from archive.org EVERY run and never read the sidecar — 598 entries, 214 with content, written nightly and never read back. So the model judged different evidence each time, and since `unknown` means both 'clean' and 'fetch failed', that silently downgraded screened domains to unscreened. Fixed (`ae0e9b3`) by the same principle as the toxic denylist one layer earlier: **remember the evidence, not just the verdict** — plus deterministic parked-page detection (6 converted on the first live run, no model call) and `scripts/spam_signatures.py`, a fingerprint net that catches the cloaked content the model misses and **works when the LLM is down**. Two real false positives are now regression tests: `especialistas` contains `cialis`, and a Marrakech clinic's `appointment slots`; note Python's `\b` treats CJK as word characters and would miss `金沙casino娱乐城`. A config error of mine was caught by measurement before shipping: `excerpt_max_age_days: 90` would have delivered ~5% of the fix because only 12 of 249 excerpts fall inside 90 days — these are DROPPED domains, so captures are old by definition. **1855 tests passing** (1395 that morning). **Still open: `Clean`/`Promising` verdicts can be produced by ABSENCE of data** — the only Clean in 247 scored 87 solely because OPR and CC were both null and nulls are excluded from the average, while every fully-enriched domain topped out at 61, and archive pages are minted from those verdicts. Full detail in the "2026-09-21" section at the end of this file. Earlier (2026-09-20) — **Common Crawl refresh automated; a 4-month-stale 0.30-weight scoring input is current again (2026-09-20, later).** `cc_source_domain_count` carries scoring weight **0.30, equal to `wayback_snapshots`**, but `cc_refresh.py` had **no timer** and had run **exactly once, by hand, on 2026-05-13**. It sat stale through **four** missed releases and nothing alarmed, because a stale-but-present SQLite returns perfectly plausible numbers. Release **`cc-main-2026-jun-jul-aug`** is now pulled, derived, verified and live: **20m50s** end to end, **119,722,885 rows**, 6.38 GB derived. **A load-bearing fact was wrong**: CC publishes the domain graph **MONTHLY** as a rolling 3-month window, not ~3x/year — all six 2026 windows exist upstream. `STRATEGIC_NOTES.md` had it right in May; this file did not. That correction is why the timer is **weekly**, not monthly: a no-op tick costs two HEAD requests, and weekly gives a failed or runtime-capped run an automatic retry instead of leaving a 0.30-weight input stale for a month. `systemd/domainsifter-cc-refresh.timer` (Sun 18:00 UTC) → `scripts/run-cc-refresh.sh` → `cc_refresh --auto`. Three safety properties, in order of importance: (1) the config swap happens **only after the derived SQLite is verified as it exists in R2** — re-downloaded, row count against a floor, known-apex canaries present/absent — so a failure leaves `latest_release` on the previous release: degraded, never half-swapped mid-run; the verification download doubles as a **cache pre-warm** so the 09:00 run never pays a 6.6 GB inline download. (2) The job **cannot still be running at 09:00 UTC** — enforced by `TimeoutStartSec=10h` (kill at Mon 04:00, 5h clearance), **not** by hoping 18:00 is early enough. Note `RuntimeMaxSec` does **not** apply to `Type=oneshot` and `TimeoutStartSec` defaults to **infinity** there, so the intuitive setting would have given no cap at all. (3) `Persistent=` is **deliberately absent**, unlike the other two timers, because a boot catch-up fires at an arbitrary time — possibly mid-pipeline — and would push to origin/main inside the daily run's push window. **Three defects fell out of the work.** The disk pre-flight check demanded **25 GiB against a measured ~38 GB peak** (DuckDB spills ~28 GB under the deliberate 8 GB memory limit); between those two numbers it passed and then died on ENOSPC hours in. It is now derived from the upstream `Content-Length` x a config multiplier. `newsletter.intro_text` hardcoded the release string (now a `{cc_release}` placeholder). `DEFAULT_INTRO` still carried the "last 3 months" claim that `7755c61` removed from config only. **The anti-recurrence part matters most**: `send_report.py` now carries a CC freshness line and escalates the subject on **🚨 CC DATA STALE** / **🚨 CC REFRESH FAILED**. The 2026-05-13 strategic note predicted the cron's weak spot would be exactly a silent failure; the 4-month drift proved it. **Retention decided**: prune raw to the newest 2 releases, **never** delete derived — raw only builds the derived SQLite and is free to re-download from CC indefinitely, while derived is what the decay-velocity thesis actually queries; 5-year projection ~$14/mo → ~$6/mo with zero loss of capability. **A signal-quality caveat worth watching**: upstream edges have been shrinking hard (18.2 → 9.4 GB since feb-mar-apr) and the damage is **not uniform** — google.com/github.com/wikipedia.org are flat (1.00x), the 0–99 buckets are ~0.98, but the **≥1000-source-domain population collapsed from 842,163 to 222,959 (0.265x)**. Day-to-day scoring barely moves because dropped domains live in the low buckets, but the signal is degrading precisely in the high-authority tail that would mark a genuinely valuable find. **1557 tests passing (+162).** Rolled out through the real automation rather than by hand: the blackout interlock was verified refusing a run at 15:31 UTC, then the 16:01 UTC service run verified in R2, swapped the config, committed `2e0635f` and pushed — 55s, because the artifacts were already uploaded. **The four stale months were then backfilled and nothing was permanently lost** — CC keeps its hyperlinkgraph releases indefinitely, so `mar-apr-may`, `apr-may-jun` and `may-jun-jul` were all still downloadable. 90 min, `rc=0`, and all three were verified against R2 **before** any raw was pruned (the backfill path does not verify, so raw was the only cheap rebuild path for them). R2 now holds **unbroken monthly coverage Feb-Aug 2026** — 5 derived SQLites, 50.02 GiB total under `cc/`, with raw pruned to the newest 2 and **every** derived SQLite retained. **Be precise about what that is**: a growing archive of monthly snapshots, NOT a per-domain history database — `cc_backlinks` still reads only `latest_release` (Strategy A), so the decay-velocity capability is enabled but **unbuilt**; the design is in `STRATEGIC_NOTES.md` under "Common Crawl backlink history", planned and deliberately not built. A trap recorded there: monthly releases are rolling 3-month windows, so adjacent releases share two thirds of their data and are not independent observations. **Then the archive was made USABLE, not merely stored (same day, third pass).** Automating the refresh left R2 holding five monthly derived SQLites while the pipeline read exactly one of them — an archive you only read the newest slice of is a cache with extra steps. Each domain now carries `cc_backlink_history`: its inbound source-domain count across **every** release we hold, newest first, auto-discovered by listing `cc/derived/` in R2 (no hand-maintained release list — a manual list is exactly what went four months un-updated) and capped by `cc_backlinks.history.max_releases`. **DISPLAY ONLY, and that boundary is load-bearing**: `cc_source_domain_count` keeps its meaning and its 0.30 weight, the new field feeds no filter/score/verdict, and regression tests pin that adding a history leaves `score`, `verdict` and published order byte-identical across good, empty, null and 13 malformed shapes. Measured live on the box: pre-warm of 4 releases in **2 minutes** (~215 MB/s from R2), then **200 candidates x 5 releases in 20 ms** — 0.10 ms/candidate, so a 2,500-candidate day costs about a quarter second. github.com reads 752,451 · 748,920 · 751,171 · 750,824 · 747,095 across the five. **Two silent config-dependent bugs fell out**: `prune_local_cache` kept only the active release, so every install would have deleted the archive cache and forced a ~30 GB re-download INSIDE the next 09:00 run (it now keeps the whole window, and keeps EVERYTHING when the window cannot be determined — including the case where a fail-soft one-entry window is indistinguishable from an R2 listing failure); and `install_release` rewrote `latest_release` on disk while the in-memory dict kept the old value, which at a small cap would have pre-warmed the superseded release and let the pruner delete one the window wants. The site gained a hand-rolled SVG sparkline in the existing Backlinks column (**hard rule 19 waived by the owner for that one file**) which never interpolates a null, claims no trend percentage because adjacent releases share two of three months, and was proven invisible for rows lacking the field by diffing the rendered `dist/index.html` against `HEAD` — zero differences. **1677 tests passing (+282 today).** Full detail in the "the archive is now actually used" section at the end of this file. Earlier the same day — **First clean run on the new stack; toxic verdicts made permanent (2026-09-20).** The ranker scored **12,997 of 13,000** names with **zero chunk failures** (vs 10,497 and two failures the day before), the toxic gate evicted at publish time for the first time, and the archive generator produced **14 pages** after two months dead (113 → 127). The newsletter's credibility line went live with the real number: *Scanned 179,215 dropped domains today*. **The run also exposed a real defect:** `unknown` means BOTH 'not abusive' and 'the archive.org fetch failed', and verdicts were recomputed from scratch each run — so `ridgemotorsports.net`, correctly classified toxic on 09-19, came back `unknown` on a failed fetch, stayed published, and was given a PERMANENT archive page; its archived content is an Indonesian online-slot gambling site. Three domains evicted and their pages deleted, and `scripts/toxic_denylist.py` now keeps an append-only record in private R2 of every domain ever classified toxic, wired into the pipeline, classify_carryover, the archive generator and the daily report. **Binding constraint now: archive.org reliability** — all 14 new pages came out in NO-EXCERPT mode because no archived content could be fetched, and 184 of 289 fetches failed on the 09-19 sweep. That caps both page quality and toxic-screen coverage. **Also open: Common Crawl is 4 months stale and was never automated** (`cc_refresh.py` has no timer, has run once, by hand) while `cc_source_domain_count` carries scoring weight 0.30. **1395 tests passing.** Full detail in the 2026-09-20 section at the end of this file. Earlier (2026-09-19) — **Toxic gate ARMED, 22 spam-squatted domains evicted, content engine restarted (2026-09-19).** The ranker drove the published list for the first time since 2026-07-23, but scored only 10,497 of 13,000 names — two 800-name chunks died on malformed JSON and four returned short, so 2,503 names became below-gate unjudged; fixed with JSON salvage, a half-size retry pass, and chunk_size 800→300. The toxic screen's first 3 verdicts were verified correct BY HAND and all 3 were already live on the site: a Chinese gambling page, a Chinese adult portal, and a REAL African safari business whose pages carried injected lottery-gambling SEO — **none catchable by the keyword filter, every name innocuous**. Gate armed; a `classify_carryover` sweep then evicted **22 of 289**, taking the list 292 → 270. Separately, the reason the site does not rank: Search Console shows **138 impressions / 8 clicks since February**, because `archive_generator.py` — the site's only long-tail content source — produced 113 pages and stopped on 2026-07-21. It has fired daily ever since via `OnSuccess=` and died on the dead API key. It now runs on the subscription backend and, crucially, states what each site ACTUALLY was from its archived title/headings instead of speculating from the name; that evidence is persisted into the page's own frontmatter (7 new optional keys) because the daily JSON and sidecar are 14-day rolling windows. `run-daily.sh` also stops deleting the excerpt sidecar every morning. **1200 tests passing, site builds.** An archive timer is written but NOT YET INSTALLED — see the operator step in the 2026-09-19 section at the end of this file. Earlier (2026-09-18) — **LLM stages rebuilt on the Max subscription; awaiting deploy, first live run in SHADOW (2026-09-18).** `scripts/llm_backend.py` (new) is the single seam: `config.json` → `llm.backend` selects `claude_code` (subscription, no per-token cost) or `api` (the metered path, kept bit-for-bit for one-value rollback). The classifier is now batched (20/call, v1→v2), the ranker chunked at 800 with 3-way parallelism and the budget/cost-meter skipped on the free backend, `classify_carryover` is backend-aware and forces shadow OFF, and `pipeline.py` finally threads `config` into the classifier (without which config edits never reached that stage). `send_report` now escalates the SUBJECT on an exit-0 run for an all-unknown classifier (`🚨 TOXIC SCREEN OFF`), a ranker fallback, or shadow mode (`👁 SCREEN IN SHADOW`, a deliberately distinct banner) — the direct fix for the 8-week silent miss. **1110 tests passing.** Live rehearsal on OVH as the service account: the classifier labelled invented pharma-spam `toxic`, a parking page `parked`, a real business `legitimate`, a blank page `empty`; the ranker scored 30 names in 2 parallel chunks in 7.0s at `cost_usd=0.0`, putting exactly the 9 seeded junk names below the gate. **Not yet on origin** — the server does `reset --hard origin/main`, so none of it is live until pushed, and `snapshot_classifier.shadow: true` means the first live run evicts NOTHING by design. Full detail in the "2026-09-18" section at the end of this file. Earlier (2026-09-17): **Claude Code subscription backend staged; both LLM stages still DOWN (2026-09-17).** The Anthropic API credit balance hit zero on **2026-07-23**, and both LLM stages have been failing soft ever since: the Phase 2 ranker falls back to mechanical selection every run, and **the `snapshot_classifier` toxic-domain screen has been off for ~8 weeks** (every domain returns `unknown`). Neither breaks a run, so nothing alarmed. Credits will not be refunded — DomainSifter has no revenue — so both stages are being moved onto Mario's €120/mo Max subscription via Claude Code (`claude -p`) instead of metered API tokens. **Phases 0–1 are complete and validated; NO pipeline file has been modified yet.** `claude-code` 2.1.267 is installed on OVH from Anthropic's signed apt repo (fingerprint verified), the subscription token is in `/etc/domainsifter/claude.env` (0640 `root:domainsifter`, **expires ~2027-09-17**), and a live test as the `domainsifter` service account scored 200/200 names into valid JSON on `claude-haiku-4-5`. Calibration: **29.4 output tokens/name → ~800-name chunks, ~16 calls/day, ~15–20 min at 3–4 parallel**; `MAX_THINKING_TOKENS=0` is required (thinking defaults ON and is pure waste here); **the real `RANKER_SYSTEM` prompt is mandatory** — a paraphrase asking about "registrability" was refused outright as a trademark question, so prose-instead-of-JSON must trip the existing fallback. Next: Phase 2 (toxic screen first, behind a `"backend": "api" | "claude_code" | "off"` config switch, shadow mode for one live run), then Phase 3 (ranker), then Phase 4 (surface backend + fallback reason in the daily report so a silent 8-week degradation cannot recur). Full detail in the "2026-09-17 — Claude Code subscription backend staged" section at the end of this file. Also resolved this period: the **2026-08-09 GitHub PAT expiry caused a 9-day publish outage** (runs completed but `git push` failed with exit 128; site froze on 2026-08-09 data) — fixed 2026-08-19 with a new 1-year PAT and the stranded commit pushed; see the Calendar section. Earlier (2026-06-02): **Permanent domain archive added (2026-06-02).** Every RDAP-confirmed-AVAILABLE domain per run — regardless of score or publication, including the low-scoring tail the publish floor drops — is now scored + verdict-stamped and written to a PRIVATE R2 archive at `state/domain_archive/YYYY-MM.jsonl` (monthly partitions). Event-based and APPEND-ONLY, keyed `(domain, availability_confirmed_date, source)`: a domain reappearing in a later run is a new dated event, never an overwrite, and re-check `availability_status` events can be appended later with no schema change. **NEVER aged out** — explicitly distinct from the 14-day-aging `state/phase2_overflow.jsonl` work queue (left untouched). Mechanism: the pipeline emits the full available-set to a gitignored local handoff (`scripts/state/available_set_latest.jsonl`) in a non-fatal try/except after `write_output` (published scoring/output path byte-identical; only the rejected tail gets newly scored, inside the archive builder); `scripts/domain_archive.py` then appends it to R2, chained as an inline non-fatal step in `run-daily.sh` (Step 3c, after publish + newsletter — same idiom as the newsletter, can't break/delay the run). `source` flag: `live` = complete available-set from a real run; `backfill` = published-subset-only reconstructed from git history of `daily-domains.json` (which never held the full available-set). **OPERATOR ACTION:** the one-time `python -m scripts.domain_archive --backfill` seed (656 deduped published events, 2026-04→2026-06, `source=backfill`) must be run MANUALLY on the OVH box where R2 creds live — it was NOT run from the dev environment (no creds). `source=live` capture begins automatically from the next cron. 923 tests passing (+21 in `tests/test_domain_archive.py`). **RDAP availability throughput raised with a safe back-off rule (2026-06-02).** Per-host runtime budget DOUBLED: `availability_check.max_runtime_per_host_seconds` and the aligned top-level `availability_budget_seconds` 2700 → 5400, so the bottleneck Verisign/.com bucket runs ~90 min (was ~45) and its derived cap is **~1350** (was 675). All per-host caps double proportionally because they are derived as `floor(runtime × workers / throttle)` — single knob. **The 4.0s Verisign throttle and 1-worker-per-host concurrency are UNCHANGED — we run longer, never faster.** Safety back-off (replaces the same-day hard-stop-on-first-429 which was over-tuned): a per-host **cumulative 3-strike-on-429** rule (`rdap_429_strike_limit`, default 3, tunable — set 1 to restore hard-stop). On each 429 we honor Retry-After (arm the per-host cooldown so the host pauses, then RESUMES at the 4s pace) and add one strike; strikes accumulate across the bucket and are NOT reset by intervening successes (so spacing can't evade the cap). The host stops for the run only on its **3rd cumulative strike**. **403 from any host = immediate catastrophic hard-stop + CRITICAL alarm** (no strike accounting — one is enough; it is the IP-block case, e.g. the GMO outcome). Logic in `scripts/enrichment/_circuit_breaker.py` (`HostStrikes` / `HostStop` / `HostCooldown`); orchestration counts `stopped_unchecked` and logs the loud per-host STOP line in `pipeline._check_availability_concurrent`. Daily report (`send_report.py`) surfaces `RDAP 429 strikes` (total, so "1 strike recovered" days are visible) and `RDAP 429 stops` (hosts that hit the limit), and escalates the email subject on any 403. WHY this is safe: per RFC 7480 §5.5 a 429 is the survivable "decrease your rate" signal, not catastrophic; Verisign publishes NO numeric RDAP limit so the real ceiling is empirical (feel it out via 429s) — see auto-memory `reference_verisign_rdap_no_published_limit`. UA on every RDAP request is the identifying `DomainSifter/1.0 (+https://domainsifter.com; contact: hello@domainsifter.com)`. Touches ONLY the RDAP availability / circuit-breaker / config / report paths + their tests — zero changes to Phase 2 ranking, scoring, or the frontend. **Phase 2 LLM name-quality ranker SHIPPED LIVE** for tomorrow's 09:00 UTC cron (merge `1a8cfa1` on `origin/main`, 2026-06-01). New pipeline stage inserted between the lexical filter and the RDAP availability check: pre-narrows lexical survivors by the existing trigram+length mechanical signal to what the daily Haiku budget can rank, scores each candidate 0-100 for "registrability as a brand", gates at score ≥ 60, and feeds the above-gate set to RDAP sorted by score desc. Replaces the prior per-host random-shuffle trim with quality-driven selection. Hard daily ceiling on Haiku spend lives in `config.json` → `phase2.daily_budget_eur` (default €3); two-layer enforcement — a planning-layer pre-narrow that sizes input to fit the cap, and a runtime cost meter that halts further batches at 95% of the cap. **Mandatory bit-for-bit fallback to the prior mechanical selection** on any failure: API error, cost-ceiling-hit-before-min-eligible, uncaught exception, missing `ANTHROPIC_API_KEY`, or above-gate count below `phase2.min_eligible` (default 10 — set low so thin-but-valid days still drive the ranker rather than reverting to mechanical). Fallback parity is the load-bearing safety property and is guarded by a dedicated regression test (`test_bucket_and_cap_uses_random_shuffle_when_no_phase2_score`) — when the fallback path runs, `_bucket_and_cap_for_availability` sees no `phase2_score` field on any candidate and runs its existing `random.Random(seed).shuffle()` path identically to today's production. Above-gate candidates that don't fit the day's RDAP budget are persisted to R2 at `state/phase2_overflow.jsonl` with `first_seen_date`, aged out at 14 days — same rolling window as carryover; mechanics only, not committed to the repo. Config knobs all tunable without code changes: `enabled`, `daily_budget_eur`, `score_gate`, `min_eligible`, `batch_size`, `concurrency`, `max_retry_passes`. 876 tests passing (+24 new in `tests/test_phase2_ranker.py`); zero regressions. **Tomorrow's 09:00 UTC cron is the first live Phase 2 run.** The metric to watch in the email report is the above-gate (≥60) percentage on real dropped domains — that's the calibration signal for whether the 60 gate is right or wants tuning. Expected log markers: `Phase 2 ranker ENABLED` through `MODE=RANKER → feeding N score-ordered candidates to RDAP bucket-and-cap`, or `Phase 2 ranker FALLBACK — <reason>` if it didn't drive. Either way the published list is valid (fallback parity guarantees it). **Same window operational change: `.shop` moved from `tlds.approved` to `tlds.pending` on 2026-06-01 (merge `6a64b92`).** Today's 09:00 UTC cron spent ~45 min (09:43–10:27) fighting `rdap.gmoregistry.net` 429 cooldowns for 0 available domains from the GMO bucket (0 available / 23 not_available / 275 unknown) — the IP-block whitelist request to GMO Registry (sent 2026-05-30) is still pending, decision point ~2026-06-06. Verified via IANA RDAP bootstrap that `.shop` is the only currently-approved TLD routed through `rdap.gmoregistry.net`; the other 45 GMO-served TLDs are brand/geo TLDs DomainSifter has no CZDS access to. Active TLD count now **14** (was 15); `.shop` returns to approved when the whitelist is granted. Carryover `.shop` entries already in `daily-domains.json` age out naturally across the 14-day rolling window; no backfill needed. **Strategy, moat, competitive analysis, paid-tier roadmap, and LLM cost economics** live in the private `domainsifter-engine` repo (`STRATEGY.md`), per the pointer at the top of this file. This public file captures operational facts only. See "2026-06-02 — Phase 2 ranker shipped; .shop moved to pending" section just above the dated-session block for the full mechanics. Earlier (2026-05-31): **State refresh** covering ~2 weeks of work that wasn't captured here. `.com` is LIVE in the daily pipeline since ~2026-05-27/28 (`tlds.approved` now 15 — added 2026-05-26 commit `582a49b`, merged to origin on 2026-05-27 via recovery merge `4ae36f8` after a same-day push-and-merge slip noted in the agent's memory; cold-start day correctly wrote the R2 baseline and reported 0 .com drops as designed by `scripts/diff.py`). Memory peak under .com load: ~47 GB on the 128 GB OVH KS-6 box — well within budget, no GC pressure or DuckDB-spill issues. Wall-clock under .com load: ~1.5h end-to-end (was ~1.0h pre-.com). **Daily cron shifted 06:30 UTC → 09:00 UTC on 2026-05-31** (timer commit `4966380`) after the first .com cron at 06:30 returned a stale/zero-diff .com zone — Verisign's daily publication doesn't reliably complete by 06:30 (171,525,128 apex names exactly matched yesterday's set, byte-identical). 09:00 UTC gives margin past the publish window; subsequent runs returned real .com diffs in the expected 126k–258k/day range. Hard rule 20 in CLAUDE.md still binds (don't move earlier than 06:30 without registry-cooldown verification). Published count now in the ~190–215/day range, mostly carryover + ~10 fresh — within expected envelope as the front-of-house display caps (per-TLD top-8 from 2026-05-25 merge `eebf17d`) shape the visible list. **GMO Registry RDAP block (rdap.gmoregistry.net) diagnosed as IP-LEVEL at the Cloudflare edge** against our OVH IP — verified the same request from a different IP returns clean 404, both IPv4 and IPv6 blocked, and the block persists regardless of throttle or 429-honoring code paths (the 2026-05-22 fix in `feat/rdap-429-honor-retry-after` is still right, it just can't recover an IP block). Whitelist request emailed to info@gmoregistry.com on 2026-05-30 with reproduction evidence; ~7-day response window, revisit ~2026-06-06. Fallback plan if declined: move GMO-served TLDs (.shop primarily, ~3% of volume and mostly low-quality candidates) to `tlds.pending` and replace with other CZDS-approved TLDs (.io / .co are leading candidates) rather than dropping them outright. **Two external-infrastructure failure modes are now DAILY, not occasional:** web.archive.org connection-refused failures during the ~10:15–10:40 UTC enrichment window land large fractions of candidates as `wayback_unknown` (external degradation; possible mitigation is moving Wayback enrichment off the hot path so a Wayback outage doesn't degrade today's list); crt.sh circuit-breaker trips most days on 502s, same class of issue. Both flagged for design attention; neither is blocking. **Paid-tier R&D continues in a separate private repository (`domainsifter-engine`)** — isolated from this pipeline, not invoked from production, touches nothing in this repo's data path. Strategy, moat definition, and competitive analysis are kept in private artifacts and are not duplicated here. **Roadmap gate — NEXT after the next clean daily run: Phase 2 LLM-based desirability ranker (Haiku-tier)** inserted between the lexical filter and RDAP, ranking candidates by name quality so the RDAP budget is spent on the best candidates instead of the current trigram-prior trim. Phase 2 gates further paid-tier work; downstream sequencing is kept in private planning. Principle reaffirmed: **prove the cheap thing before building the expensive thing around it.** **Calendar / pending:** GMO whitelist response decision point ~2026-06-06; `.biz` CZDS reapply window ~2026-07-15; GitHub PAT rotation ~2026-08-09; DMARC progression still at `p=none` (was due for review per the 2026-04-30 follow-up); Gmail spam flag on daily operational report unresolved (mark-not-spam workaround still in use). See "2026-05-31 — State refresh" section just above the dated-session block for the full per-topic breakdown; everything below is preserved historical context. Earlier (2026-05-14): Common Crawl backlinks wired into the daily pipeline — `cc_backlinks` is now registered in `ENRICHMENT_MODULES`, `cc_source_domain_count` is part of the locked JSON contract (PLAN.md Principle 5), the homepage shows a new Backlinks column, and the methodology section has a new Step 5 card ("Check backlinks"). Scoring weights gained `cc_source_domain_count: 0.30` — symmetric with `wayback_snapshots: 0.30` because PLAN.md frames both as historical-authority signals (Wayback = temporal evidence, CC = link evidence). Log-scaled with /4.0 divisor (saturates at ~10,000 source domains — fatter divisor than Wayback's /3.0 because CC's distribution has a heavier tail). Null handling: `cc_source_domain_count=null` (apex not in latest release's graph) is excluded from the scoring average AND not counted toward `publish_min_enrichment_completeness`, so absence from CC doesn't penalize a candidate. Tomorrow's 06:30 UTC OVH cron is the first production run that exercises CC backlinks in scoring. Strategy A (latest release only) — see STRATEGIC_NOTES.md `Multi-release CC query strategy` for B/C deferral. See "Common Crawl wire-in — 2026-05-14" section at end of file for the full surface (touched files: scripts/score.py, scripts/output.py, scripts/pipeline.py, scripts/config.json, scripts/enrichment/__init__.py, scripts/enrichment/cc_backlinks.py, src/components/DomainTable.astro, src/components/Methodology.astro, src/data/sample-domains.json, tests/test_score.py, tests/test_output.py, tests/test_pipeline.py, tests/enrichment/test_cc_backlinks.py, plus PLAN.md/STATE.md/STRATEGIC_NOTES.md). Same-day second feature: daily newsletter draft generator (`scripts/generate_newsletter.py`) — reads daily-domains.json, builds an HTML email of top-20 picks (per-row anchor links to the live site, UTM-tagged affiliate registrar logos, teal accent matching the site), POSTs to Buttondown as a draft. Drafts NEVER auto-send: Mario tests via dashboard "Send draft" to himself, then flips status to `about_to_send` when satisfied. Disabled by default (`config.newsletter.enabled=false`); flip to true after adding `BUTTONDOWN_API_KEY` to OVH .env. Idempotent (same-day re-run finds existing draft by subject and skips create). 48 new tests in `tests/test_generate_newsletter.py`. Per-row anchor IDs (`id="drop-{name}"`) added to DomainTable.astro desktop rows so email-link clicks scroll directly to the matching row on the live site. See 'Daily newsletter — 2026-05-14' section at end of file for the full operator workflow. End-of-day: 12 commits landed today (1 autonomous OVH daily-refresh `389f300`, 1 CC wire-in `bae0bde`, 1 methodology copy fix `554d604`, 7 table-layout iterations settling on a 3-dot teal divider + `max-w-[1175px]` section + duplicate-Listed-column removal, 1 newsletter generator `896b45e`, 1 newsletter-enable flag flip `ebaa94a`). Today's 06:30 UTC autonomous pipeline run: exit 0, 2h 12m 44s wall-clock, **72 domains published (20 new + 52 carryover)**, memory peak **3.86 GB (first time the cgroup-direct read produced real data — the 2026-05-12 fix is working)**. DNS pre-filter day 2 of the new behaviour profile: **0% rejection / 54% unknown** — under observation through Sunday 17 May, with disable-vs-investigate decision pending. Gmail flagged this morning's daily report email as spam (concerning but separate from the pipeline; mark Not Spam + watch). Newsletter infrastructure live but no draft created yet — awaiting Mario's manual run on OVH after he adds `BUTTONDOWN_API_KEY` to .env. 514/514 tests passing (+48 from newsletter suite). See 'Day-end summary 2026-05-14' section at end of file for the full inventory + production state + pending decisions. Yesterday (Evening update: Common Crawl standalone capability shipped, first operational `cc_refresh` run on OVH completed successfully, and a same-day hotfix unblocked it. Three new commits today after this morning's Buttondown doc-sync: `ca38e6f` shipped `scripts/cc_refresh.py` + `scripts/enrichment/cc_backlinks.py` + 40 new tests + duckdb dep + STRATEGIC_NOTES.md initialisation; `33845a0` pushed after rebase over today's autonomous daily refresh; `f9887de` hotfix replaced `storage_class="INFREQUENT_ACCESS"` with `"STANDARD_IA"` after Mario's first OVH run hit `InvalidStorageClass` — the agent had used Cloudflare's Workers-API spelling instead of R2's S3-API spelling. Post-fix run on OVH completed end-to-end: release `cc-main-2026-feb-mar-apr` now in R2 (raw vertices+edges at `cc/raw/cc-main-2026-feb-mar-apr/*.txt.gz` on `STANDARD_IA`, derived SQLite at `cc/derived/cc-main-2026-feb-mar-apr.sqlite` on Standard). Validation queries returned plausible real-world numbers: `google.com → 16,365,926` source domains, `github.com → 747,095`, invented test names (`marketglow.com`, `tideblock.com`) returned "not in CC graph" as expected. `cc_backlinks` is still NOT registered in `ENRICHMENT_MODULES` — wire-in is the next commit. Also today: first organic newsletter subscriber (`damnv724@gmail.com`) validated Buttondown integration end-to-end under real production traffic — count 1 owner test → 2 subscribers including first real user. Seven pending decisions captured in `STRATEGIC_NOTES.md` (multi-release CC query strategy, free/paid tier model, daily publication count cap, CC refresh cadence). See "Day-end summary 2026-05-13" section at end of file for forward plan (14-17 May: observe → wire CC → observe → re-enable .com). Yesterday's three fixes (DNSBL three-state, memory peak cgroup, DNS pre-filter) summary preserved below for context. Yesterday's lead: First fully-autonomous OVH run at 06:30 UTC fired cleanly but produced zero new domains. Investigation surfaced a coupled environmental + code issue: OVH's shared DNS resolver `213.186.33.99` is rate-limited by Spamhaus and SURBL fair-use policies, so Spamhaus returned `127.255.255.254` ("query via public resolver, refused") for every lookup and SURBL returned NXDOMAIN for every lookup. The 127.255.255.254 response was then *mis-coded as a real listing* by `scripts/enrichment/_dnsbl.py`, causing 100% post-enrichment rejection. Two-part fix this commit: (1) OS-level — `/etc/systemd/resolved.conf` set to Quad9 primary (9.9.9.9, 149.112.112.112) with Cloudflare/Google fallback, `systemd-resolved` restarted, manually verified DNSBL queries now return correct codes; (2) code — `_dnsbl.py` now distinguishes three states (listed=True for 127.0.0.x/127.0.1.x, listed=False for NXDOMAIN, listed=None for the error band 127.255.255.x and any unexpected response or transport failure), `spamhaus.py`/`surbl.py` pass the None through as `{"spamhaus_listed": None}` / `{"surbl_listed": None}` instead of collapsing to empty dict, `filter.py` already rejected only on `is True` (so None and missing now correctly pass through), and `filter_candidates_post_enrichment` logs a per-run `DNSBL signal distribution` line so degraded-resolver days surface in the email report. **OVH DNS resolver: Quad9 (set 2026-05-12).** Tests: new `tests/enrichment/test__dnsbl.py` with 13 cases covering all three states + edge cases; `test_spamhaus.py` / `test_surbl.py` / `test_filter.py` updated for the new contract. See "DNS resolver fix and DNSBL three-state contract — 2026-05-12" section at end of file for full diagnosis and the two known risks. Second fix today (afternoon): email reporter's "Memory peak: (unavailable)" output traced to a race between the EXIT trap's `systemctl show -p MemoryPeak` and systemd's unit teardown — fixed by reading `/sys/fs/cgroup/<unit>/memory.peak` directly from `scripts/run-daily.sh` and exporting `DOMAINSIFTER_MEMORY_PEAK_BYTES` for `scripts/send_report.py` to consume; systemctl path retained as fallback for non-wrapper invocations. Enables empirical memory-peak measurement needed before re-enabling `.com` in `tlds.approved`. See "Memory peak instrumentation fix — 2026-05-12" section at end of file. Third fix today (evening): RDAP throttle budget was the sole remaining blocker for re-enabling `.com` in `tlds.approved` — today's saturated run logged 214 candidates skipped (budget) across 8 host buckets even on the 14-TLD set, and adding .com (~10× the next-largest TLD's apex count) under today's per-host throttles would have made the saturation orders of magnitude worse. Architectural fix: new DNS pre-filter stage in `scripts/dns_prefilter.py` runs AFTER lexical filter and BEFORE RDAP bucket assignment; for each candidate apex, queries NS records via the system resolver (Quad9 since this morning) — NXDOMAIN means the registry has removed delegation (proceed to RDAP), NS records present means the domain is still delegated (reject pre-RDAP), error states (timeout/NoAnswer/transport) fail open to RDAP. Expected to reject 80-95% of post-lexical candidates with a free, fast, parallelisable lookup; accuracy-preserving (every candidate RDAP would have approved as available also has no NS records, so DNS pre-filter rejects only candidates RDAP would have rejected too). Configuration in `dns_check` section of config.json — conservative defaults of 20 workers, 3s timeout, 0s throttle on first deployment. Unblocks `.com` re-enablement (still wants one observed clean run + an RDAP throttle review, but the DNS pre-filter is the precondition that makes both possible). 17 new tests across `tests/test_dns_prefilter.py` (new file, 16 cases) and `tests/test_pipeline.py` (1 integration test asserting registered-domain rejection happens before RDAP). New dependency: `dnspython==2.6.1` (small pure-Python BSD lib; second focused-library exception to the stdlib-only stance after the `wayback` package on 2026-05-08). See "DNS pre-filter pipeline stage — 2026-05-12" section at end of file for the full architecture and the .com unblocking story.))
...
> **Where strategy lives:** Strategic direction, moat definition, competitive analysis, and paid-tier roadmap are maintained in the private `domainsifter-engine` repo (`STRATEGY.md`) — not in this public file. Read that for the full strategic picture.
...
- **Approved and LIVE in `tlds.approved` (15, as of 2026-05-27):** .app, .dev, .live, .studio, .tech, .online, .site, .store, .xyz, .info, .org, .shop, .biz, .net, .com
- Pending: none — all originally-requested TLDs are now live in the pipeline
- Renewal calendar: `.biz` reapply window ~2026-07-15 (CZDS administrative renewal, not first-time approval); other TLDs expire 2036–2037 with auto-renew enabled
...
> Historical access-approval timeline (kept for context): the initial 11-of-15 approval ladder (April 2026) → .com/.net/.shop/.biz approved later that quarter → all 15 live as of 2026-05-27 with .com cold-start. See dated sessions below for the .com rollout.
...
## Day-end summary 2026-05-14

Today produced two product milestones (CC backlinks wired in, daily newsletter shipped), one operational event (autonomous pipeline run executed cleanly at 06:30 UTC), and seven iterations of layout polish on the homepage table. This section is the end-of-day rollup; the long-form per-feature sections above carry the technical detail.

### Today's commits (12)

| Commit | Type | Description |
|---|---|---|
| `389f300` | data | daily refresh 2026-05-14 (autonomous OVH cron, 06:30 UTC) |
| `bae0bde` | feat(cc) | wire Common Crawl backlinks into daily pipeline |
| `554d604` | docs | use agreed methodology Step 5 body copy (original wire-in prompt had different wording by mistake) |
| `8443b1d` | fix(table) | remove duplicate Listed column, widen section to max-w-7xl |
| `ad8f252` | fix(table) | tighten section from max-w-7xl to max-w-[1175px] |
| `40a486e` | feat(table) | teal section kickers + wider gap between cards (later reverted) |
| `a9d8681` | fix(table) | replace teal kickers with horizontal divider between cards |
| `61a4f89` | fix(table) | make card divider teal, thicker, centered at 75% width |
| `ad75923` | fix(table) | soften divider — pill shape, center-fading teal gradient |
| `7d4a758` | fix(table) | swap gradient line for three teal dots (chapter-break style) |
| `896b45e` | feat | newsletter — daily Buttondown draft generator from pipeline output |
| `ebaa94a` | chore | flip `newsletter.enabled` to true (drafts are inherently safe) |

### Today's shipped work (5 streams)

**1. Common Crawl standalone capability fully validated on OVH.** Yesterday's commits (`33845a0` / `f9887de`) shipped the standalone capability. Validation cycle confirmed cleanly: `cc_refresh` execution produced the first CC release in R2 (`cc-main-2026-feb-mar-apr`); CLI queries returned `google.com → 16,365,926` source domains, `github.com → 747,095`, and "not in graph" for invented test names. All preconditions for wiring met.

**2. CC wire-in to pipeline.** Commit `bae0bde` registered `cc_backlinks` in `ENRICHMENT_MODULES`, added `cc_source_domain_count` to the locked JSON contract (PLAN.md Principle 5), added the new field as a scoring component (weight 0.30, log-scaled with /4.0 divisor, saturates at ~10k source domains — symmetric with `wayback_snapshots: 0.30` because both signals are historical-authority complements). Homepage gained a sortable Backlinks column between OpenPageRank and Verdict. Methodology section gained a Step 5 "Check backlinks" card with the `link-2` icon. Sample data normalized to match production behaviour (`dropped_date = first_seen_date` on every entry — fictional divergence served only the now-removed Listed column). +14 tests covering scoring, output contract, pipeline integration; load-bearing assertion in `test_cc_backlinks.py` inverted to assert IS in `ENRICHMENT_MODULES`. See "Common Crawl wire-in — 2026-05-14" section above for the full surface.

**3. Layout polish on the homepage table.** Eight iterations across two themes:
- **Listed column removal** (`8443b1d`): Production data invariant — `dropped_date == first_seen_date` always, because both fields are written on the same code path (`pipeline.py:205` for `dropped_date`, `carryover.py:131` for `first_seen_date`) and never modified after. The Listed column was rendering identical values to Dropped on every row. Removed entirely from rendering (schema field stays in JSON for backward-compat); both cards now share the same column structure + widths.
- **Section width tightening** (`8443b1d` → `ad8f252`): explored `max-w-7xl` (1280px) then settled on `max-w-[1175px]` — exactly enough for the Register popover (button + 3 affiliate logos = ~210px) with ~21px breathing room past the third logo. 23px wider total than `max-w-6xl` (the rest of the site); small asymmetry that reads as "table needs the room" rather than "different layout system."
- **Card separator iterations** (`40a486e` → `7d4a758`): five iterations searching for a visual signal that catches the eye on fast scroll (especially mobile, where peripheral context is lost). Tried teal section kickers (felt like dumb duplicate text), 2px stone-300 line (almost invisible), 4px teal line (too rectangular), centered teal pill gradient (still felt like a smudge), and finally three small teal dots in the editorial chapter-break pattern. The dots stay.

**4. Daily newsletter generation infrastructure.** Commit `896b45e` shipped `scripts/generate_newsletter.py` (527 lines): reads `daily-domains.json`, takes top 20 by score, builds a 720px nested-table HTML email with inline CSS (brand teal accent, per-row anchor links to the live site, 3 UTM-tagged affiliate logos per domain), POSTs to Buttondown as a draft via `POST /v1/emails` with `status="draft"`. Idempotent by subject match. Wired into `run-daily.sh` as Step 3b — failures are logged but non-fatal. Per-row anchor IDs added to DomainTable.astro desktop rows (`id="drop-{name}"`); mobile cards intentionally skipped to avoid duplicate-IDs invalidating HTML5. +48 new tests. Commit `ebaa94a` flipped `newsletter.enabled` to true — drafts are inherently safe (Buttondown's `status=draft` never reaches subscribers without a manual dashboard action), so flipping the script flag on creates no deliverability risk. See "Daily newsletter — 2026-05-14" section above for the full operator workflow.

**5. Autonomous 06:30 UTC production pipeline run.** First clean autonomous run in several days. Metrics:

| Metric | Value | Notes |
|---|---|---|
| Wall-clock | 2h 12m 44s | Within budget |
| Exit code | 0 | Clean |
| Total published | 72 | 20 new today + 52 carryover from prior 14 days |
| Memory peak | **3.86 GB** | First time the cgroup-direct read populated a real number — the 2026-05-12 instrumentation fix is working. 3.86 GB on the 14-TLD set leaves substantial headroom (server has 128 GB) for the .com addition planned Sunday |
| DNS pre-filter rejection rate | **0%** | Day 2 of this profile — see pending decisions below |
| DNS pre-filter unknown rate | **54%** | Hypothesis: Quad9 anycast resolver may be timing out under 20-worker burst, OR there's a logic bug in the three-state mapping in `_dnsbl.py`/`dns_prefilter.py`. Under observation through Sunday |
| Email reporter | Sent | Brevo SMTP via systemd EXIT trap. But Gmail flagged it as spam this morning — separate operational issue |

The pipeline run executed BEFORE today's CC wire-in commit landed, so this run was the LAST run without CC backlinks. Tomorrow's 15 May run is the first one producing backlink-enriched output.

### Production state going forward

- **Pipeline**: 14 TLDs in `tlds.approved` (no `.com` yet), runs autonomously at 06:30 UTC via the Cloudflare Worker `domainsifter-cron-trigger` → GitHub Actions cron → OVH systemd timer. Hard rule 20 (don't move cron earlier than 06:30 UTC without verifying registry RDAP cooldown windows) remains active.
- **CC backlinks**: wired into the pipeline as of `bae0bde`. Tomorrow's run is the first one producing the new `cc_source_domain_count` field on every domain. Validation step: after tomorrow's run completes, check that `src/data/daily-domains.json` has integer values for in-graph domains and `null` for not-in-graph.
- **Newsletter infrastructure**: `scripts/generate_newsletter.py` deployed, `config.newsletter.enabled=true` in main, `BUTTONDOWN_API_KEY` pending operator add to OVH `.env`. Mario will SSH, pull, manually run `python -m scripts.generate_newsletter` to create today's first draft for dashboard QA. Tomorrow's 06:30 UTC cron will create the second draft automatically.
- **Test surface**: 514 tests, all passing locally. +48 from today's newsletter suite.

### Pending decisions

1. **DNS pre-filter fate** — observe through Sunday 17 May. Current behaviour (0% rejection, 54% unknown) defeats the original architectural goal of the stage (relieve RDAP throttle pressure by rejecting NS-record-present candidates pre-RDAP). Two branches at the Sunday decision point:
   - (a) **Disable**: remove DNS pre-filter from the pipeline. RDAP availability check handles all candidates directly. Reverts the 2026-05-12 change that added the stage. Loss: the throttle-relief mechanism. Gain: simpler pipeline, no observed benefit lost since the stage isn't producing useful signal.
   - (b) **Investigate**: examine Quad9 anycast timeout behaviour under our 20-worker burst, check three-state mapping logic in `_dnsbl.py` / `dns_prefilter.py`, possibly switch to a different system resolver or reduce worker count. Cost: 1-2 hours of debugging; might find a real bug or might confirm Quad9 just can't keep up.
   
   Lean: investigate first, disable if the cause is environmental and unfixable.

2. **`.com` to `tlds.approved`** — planned for Sunday 17 May IF pipeline is stable through 15-16 May. Memory peak 3.86 GB on the 14-TLD set means we have ~30× headroom for the ~10× volume increase from `.com`. Pre-conditions met: CC backlinks wired (adds signal density), DNS pre-filter behaviour resolved (either way — option (a) or (b) above), no critical-level log lines for two days in a row.

3. **Gmail spam flag on operational emails** — this morning's daily report email landed in Gmail spam folder. Operational concern (the operator-facing run report should always reach the inbox). Action plan:
   - Today: mark Not Spam, observe.
   - Tomorrow: if same fate, investigate Brevo deliverability — SPF/DKIM/DMARC are already in place per STATE.md "Email infrastructure," but Brevo may need a feedback loop registration, IP warm-up, or sender reputation rebuild.
   - Worst case: route operational email through a different sender (e.g. direct SMTP from OVH with self-signed DKIM) and reserve Brevo for transactional/subscriber-facing email only.

4. **Affiliate program statuses** — open thread separate from the pipeline:
   - impact.com Namecheap verification (pending since site went live; the `<meta name='impact-site-verification' value='...'>` tag is in `Layout.astro:68` but no confirmation of approval yet)
   - Dynadot affiliate status check (need to verify the `rscreg=domainsifter` parameter is being credited properly)
   - NameSilo `rid=36a0644du` parameter — confirm still active in their referral dashboard
   
   Action: separate operator session to check each registrar's dashboard. Not blocking pipeline work.

### Forward plan (refresh from yesterday)

Conditional on each previous day's run being clean before advancing:

- **Thu 15 May (tomorrow)**: observe FIRST production run with CC backlinks in scoring. Validate `cc_source_domain_count` appears on published domains; check score distribution shifts modestly. Mario manually creates today's first newsletter draft via dashboard, QAs it in his own inbox.
- **Fri 16 May**: second clean run. If both 15 and 16 May runs are clean, .com gate met.
- **Sat 17 May**: re-enable `.com` in `tlds.approved` IF pipeline stable AND DNS pre-filter decision resolved AND Gmail spam issue resolved. Three preconditions, all expected to converge by Saturday.

Each day's go/no-go gate is "previous day's email report shows clean run, no critical log lines, no surprising count distributions, and the operator inbox actually received the report (vs Gmail-spam-folder)."

### Pending decisions captured for future sessions

In STRATEGIC_NOTES.md (no NEW sections from today — yesterday's CC wire-in update already marked Multi-release Strategy A as active):

- Common Crawl integration — accumulation strategy (still current)
- Multi-release CC query strategy — Strategy A active in production
- Free vs paid tier model — pending, no change
- Daily publication count cap — pending, no change
- Common Crawl refresh cadence — pending, no change (manual until 2-3 releases land)

In STATE.md (operational pending items above):
- DNS pre-filter fate (Sunday gate)
- .com to approved (Sunday gate)
- Gmail spam flag (tomorrow observation)
- Affiliate program statuses (separate operator session)
...
## 2026-06-02 — Phase 2 ranker shipped; .shop moved to pending

State of the world as of 2026-06-02 evening. Two production changes landed since the 2026-05-31 refresh and tomorrow's 09:00 UTC cron is the validation event for the larger of them. Strategic context (paid-tier direction, moat, competitive analysis, exact LLM spend economics) lives in the private `domainsifter-engine` repo (`STRATEGY.md`) per the pointer at the top of this file. This section captures operational mechanics only.

### Operational changes

- **`.shop` moved from `tlds.approved` to `tlds.pending` on 2026-06-01.** Single-file config change (`scripts/config.json`), single-line `_doc` note recording the why. Merge `6a64b92` on `origin/main`. Reason: today's 09:00 UTC cron spent ~45 min (09:43–10:27) fighting `rdap.gmoregistry.net` 429 cooldowns and produced 0 available / 23 not_available / 275 unknown from the GMO bucket. The IP-block whitelist request to `info@gmoregistry.com` (sent 2026-05-30, see the 2026-05-31 section below) is still pending its ~7-day response window — decision point ~2026-06-06. This is a TEMPORARY move; `.shop` returns to approved when GMO whitelists our OVH IP, which is a one-line config change reverting the move. Verified via IANA RDAP bootstrap that `.shop` is the only currently-approved TLD routed through `rdap.gmoregistry.net`; the other 45 GMO-served TLDs are brand/geo TLDs we have no CZDS access to. Active TLD count now 14 (was 15). Carryover `.shop` entries already in `daily-domains.json` will age out naturally across the existing 14-day rolling window; no backfill or eviction needed. Frontend `Hero.astro` / `Methodology.astro` "across 15 TLDs" copy NOT touched — directional, and the gap is temporary; re-add restores 15. 852/852 tests passing immediately after this commit (the test fixtures in `tests/enrichment/test_rdap.py` that hardcode `"shop" → rdap.gmoregistry.net` are synthetic bootstrap data for cooldown-logic tests; they do not read `tlds.approved`).

- **Phase 2 LLM name-quality ranker SHIPPED LIVE for tomorrow's 09:00 UTC cron.** Feature branch `feat/phase2-ranker`, merge `1a8cfa1` on `origin/main` 2026-06-01 evening. New module `scripts/phase2_ranker.py`, single-block insertion in `scripts/pipeline.py` between the lexical filter and the existing `_bucket_and_cap_for_availability`, plus one conditional inside `_bucket_and_cap_for_availability` that switches from random-shuffle trim to score-desc trim when candidates carry a `phase2_score` field. New config block `phase2` in `scripts/config.json`. New test file `tests/test_phase2_ranker.py` with 24 tests. 876 tests passing overall (+24); zero regressions.

### Phase 2 ranker — mechanics (PUBLIC, mechanics only)

The strategic framing of Phase 2 — why a name-quality ranker is the gate it is, what it tests about the drop pool, and what depends on its verdict — is in `STRATEGY.md` in the private repo. This section documents the mechanics only.

**Pipeline position.** Inserts after `lexical_filter.filter_candidates` and before `_bucket_and_cap_for_availability`. Single new call: `rank_and_select(lexical_kept, config, today=today)` returns `(candidates_for_bucketing, status)`. On success, `candidates_for_bucketing` is the above-gate subset of lexical survivors with each candidate carrying `phase2_score: int` and `phase2_reason: str` fields. On disabled / fallback, it IS the original `lexical_kept` unchanged.

**Pre-narrow.** Same sort key the existing `global_cap` trim uses (`(-trigram_match_count, apex_length, name)`). Top N is computed at runtime from `phase2.daily_budget_eur` and the planning-rate knob, both in the config. With the configured defaults N is well under today's ~48k lexical survivors, so the bulk of survivors are dropped before any Haiku call.

**Ranking pass.** Haiku (`claude-haiku-4-5-20251001`), batched (`batch_size=15` post-diagnostic calibration — the smaller batches reduce JSON-tail truncation rates observed at 25), concurrency 6, single retry pass for any names missing from the model's response. Strict-JSON output: `[{"domain": ..., "score": 0-100, "reason": "3-6 words"}, ...]`. The system prompt (`scripts/phase2_ranker.py:RANKER_SYSTEM`) defines the brand-quality criteria — rewards recognizable English compounds, penalizes gibberish, foreign-language mashes, spam-vertical patterns, SEO keyword-stuffing.

**Gate.** Candidates with `phase2_score >= phase2.score_gate` (default 60) are quality-eligible. Below-gate are discarded for this run. Anything Haiku failed to score after the retry pass is recorded with `score=0` and reason `"missing from response"` — treated as below-gate, never silently lost.

**RDAP selection.** Above-gate set is sorted by `(-phase2_score, name)` and passed to `_bucket_and_cap_for_availability`. The bucket-and-cap detects the score field and uses score-desc selection per host bucket. When no score field is present (fallback path), the existing `random.Random(seed).shuffle()` runs bit-for-bit.

**Hard budget ceiling — two layers.**
1. Planning layer: `target_n` from the formula above sizes input to budget.
2. Runtime layer: a thread-safe cost meter accumulates real Haiku token usage per response; further batches are not submitted once cumulative spend crosses 95% of `daily_budget_usd`. In-flight batches finish; pending futures are cancelled. Partial-rank survivors still drive RDAP if they clear `min_eligible` (mode = `ranker_partial`); if they don't, fallback fires (mode = `fallback`, reason = `cost_ceiling_below_min_eligible`).

**Mandatory fallback — wired to FAILURE only, not thin yield.** Per product policy "smaller but cleaner is better", the ranker drives on thin-but-valid days (e.g. 20 above-gate domains) rather than reverting to mechanical selection. Fallback fires on: API error, cost ceiling hit before `min_eligible`, uncaught exception, missing `ANTHROPIC_API_KEY`, or above-gate count < `phase2.min_eligible` (default 10 — set low so genuinely-thin yields still drive). When fallback fires, the orchestrator returns the ORIGINAL `lexical_kept` unchanged (no scores attached); `_bucket_and_cap_for_availability` sees no `phase2_score` field and runs its existing `random.Random(seed).shuffle()` bit-for-bit identical to today's production. `test_bucket_and_cap_uses_random_shuffle_when_no_phase2_score` is the regression guard for that parity.

**Logging.** Stdout (which the email reporter captures) shows `Phase 2 ranker ENABLED — lexical=N, budget=€X.XX ($Y.YY), ceiling=$Z.ZZ, target_n=M, gate=G, min_eligible=E` on success entry, a 10-point score distribution band, the above-gate count, and either `MODE=RANKER → feeding N score-ordered candidates to RDAP bucket-and-cap` or `Phase 2 ranker FALLBACK — <reason>` followed by `→ reverting to mechanical selection`.

**Overflow persistence.** Above-gate candidates that don't fit any RDAP bucket (because the host's per-host cap is exhausted by higher-scoring siblings) are persisted to R2 at `state/phase2_overflow.jsonl` with `first_seen_date` set to today, aged out at 14 days (same rolling window as carryover). The file is JSONL append-only, mirrors the existing R2 state pattern in `scripts/diff.py`, NOT committed to the repo. Side-effect-only step; R2 errors are logged and swallowed, never block the daily run. Purpose: keeps quality-eligible inventory available for later consumption rather than discarding it at RDAP capacity. Mechanics-level statement; the consumer side of overflow lives in `STRATEGY.md`.

**Config knobs (all tunable in `scripts/config.json` → `phase2` without code changes).**

```
enabled                       feature flag — ships TRUE for live activation
daily_budget_eur              hard ceiling on Haiku spend per run (default €3)
eur_to_usd                    FX for the planner
cost_per_1000_planning_usd    planning rate with headroom for retry + drift
score_gate                    eligibility threshold (default 60)
min_eligible                  fallback floor — fires on broken yield, not thin yield (default 10)
model                         claude-haiku-4-5-20251001
batch_size                    smaller batches reduce JSON-tail truncation (default 15)
concurrency
max_retry_passes              one retry pass for missing names (default 1)
```

Concrete defaults for the budget / planning-rate / model knobs live in `scripts/config.json`'s `phase2._doc` block; the table above documents the schema and intent.

### Tomorrow's 09:00 UTC cron — calibration event

The metric to watch in the email report is the **above-gate (≥60) percentage on real dropped domains**. The diagnostic produced a healthy ~23% above-gate on ~1,000 currently-registered-apex names (proxy for lexical-survivor character), but real drops are a different population — known-to-be-recently-released names skew differently. Calibration outcomes:

- **Above-gate ~15–30%** → gate is correct, ranker drives a comfortable RDAP load.
- **Above-gate < ~10%** → gate is too tight. Drop to 55 the next day (`phase2.score_gate: 55`).
- **Above-gate > ~40%** → gate is too loose, the RDAP budget is wasted on borderline candidates. Raise to 65.
- **Mode = FALLBACK** → check the reason line in the log; the published list is still valid (fallback parity guarantees it) but the ranker isn't driving.

### Test surface

876 tests passing across the suite (+24 new). New tests in `tests/test_phase2_ranker.py`:

- pre-narrow correctness (sort key + truncation + budget planner conversion)
- batch cost aggregation across multiple batches
- missing-from-response recorded as below-gate (never vanished)
- retry pass recovers missing names when possible
- cost ceiling halts further batches at 95% of budget
- `rank_and_select` branches: success, disabled, too-few-eligible, exception, ceiling-below-min-eligible, ceiling-above-min-eligible (mode=`ranker_partial`), no-api-client
- `_bucket_and_cap_for_availability` uses score-desc when candidates carry `phase2_score`
- **`_bucket_and_cap_for_availability` uses random-shuffle when no scores — regression guard for fallback parity**
- `record_overflow` writes only above-gate-not-in-RDAP, ages out >14-day entries, appends today with `first_seen_date`, no-ops on fallback/disabled, swallows R2 errors
- pipeline integration: log lines surface, full-path with mocked Haiku produces the expected sorted output

All Anthropic API calls mocked (per CLAUDE.md rule #13 — no live API in tests). All R2 calls mocked via `botocore.exceptions.ClientError` for `NoSuchKey`.

### What's NOT in this section (by design — public-repo discipline)

- Strategic framing of why Phase 2 is the gate it is, what it tests about the pool, what depends on its verdict — in `STRATEGY.md`.
- LLM cost figures, full-coverage spend extrapolation, margin economics — in `STRATEGY.md`.
- Downstream consumer roles for the overflow file — in `STRATEGY.md`. The mechanic above (R2 persistence with dates, 14-day age-out) is the public-safe statement.
- Competitive positioning, moat, and related strategic framing — in `STRATEGY.md`.

---

## 2026-05-31 — State refresh (~2 weeks of work folded in)

State of the world as of 2026-05-31, organized by topic. Captures the .com rollout, the cron-window shift, the GMO RDAP block, the two new daily external-infrastructure failure modes, and the next-priority phase gate. Strategic content (paid-tier direction, moat, competitive analysis) is kept in private artifacts and only acknowledged here at the level of "private R&D continues, isolated from this pipeline." Earlier dated sessions below this one (2026-05-21 backward) remain unchanged.

### Production / pipeline state

- **`.com` is LIVE in the daily pipeline.** Added 2026-05-26 in commit `582a49b` (`tlds.approved` 14 → 15, `tlds.pending` → `[]`, `_doc` refreshed to document the re-add and the OVH KS-6 validation that enabled it). The first push slipped (committed locally, never reached `origin`), caught on 2026-05-27 and recovered via merge commit `4ae36f8` on `origin/main`. First .com-enabled cron ran 2026-05-27 — cold start, wrote the R2 baseline, reported 0 .com drops as designed by `scripts/diff.py.load_yesterday`. Pre-.com production logs were "Collected X total drops across 14 TLDs"; post-merge they read "across 15 TLDs".
- **Memory peak under .com load: ~47 GB** on the 128 GB OVH KS-6 box. Well within budget; no spill, no GC pressure. The 2026-05-12 cgroup-direct read instrumentation is the source of this number (not `systemctl show -p MemoryPeak`, which has the unit-teardown race documented in the 2026-05-12 section below).
- **Wall-clock under .com load: ~1.5h** end-to-end, versus ~1.0h pre-.com. .com zone download (~25 GB compressed) + parse (~50 GB expanded into the in-memory `set[str]`) account for the bulk of the extra time. RDAP availability and enrichment phases are unchanged.
- **Daily cron shifted 06:30 UTC → 09:00 UTC on 2026-05-31** (timer commit `4966380` on branch `feat/cron-shift-0900-utc`). The first .com-enabled cron at 06:30 returned a stale `com.zone` whose apex set was byte-identical to yesterday's (171,525,128 names exactly), producing a zero-diff for .com on a day when real drops were expected. Verisign's daily publication doesn't reliably complete by 06:30; CZDS documents a 00:00–06:00 UTC publish window but practice runs longer. 09:00 UTC gives margin past it. Subsequent runs returned real .com diffs in the 126k–258k/day range. CLAUDE.md hard rule 20 (don't move cron earlier than 06:30 without registry-cooldown verification) still applies in the other direction; 09:00 is later, not earlier, so the rule isn't engaged.
- **Published count: ~190–215/day**, mostly carryover from the 14-day rolling list + ~10 fresh entries per day. Within expected envelope given the front-of-house per-TLD top-8 display cap (2026-05-25 merge `eebf17d`) shapes which entries are visible on the homepage's two panels.
- 852/852 tests passing across the suite as of the last touch in this period.

### GMO Registry RDAP block — diagnosis complete, awaiting whitelist

- `rdap.gmoregistry.net` returns instant HTTP 429 to our OVH IP at the Cloudflare edge. **Diagnosed as an IP-level block**, not a rate-limit or a config issue. Verification: the same RDAP request issued from a different IP returns clean HTTP 404; both IPv4 and IPv6 from our OVH host are blocked; the block persists regardless of throttle or 429-honoring code path. The 2026-05-22 per-host 429 cooldown fix (`feat/rdap-429-honor-retry-after`) is still correct — it just can't recover an IP block.
- Effect on production: the GMO RDAP bucket goes ~100% unknown daily. Candidates routed to that bucket are rejected at the availability gate. Operational impact is small (GMO-served TLDs are a low-volume slice).
- **Whitelist request emailed to info@gmoregistry.com on 2026-05-30** with reproduction evidence (request headers, IP, time, observed response). ~7-day response window. Decision point: ~2026-06-06.
- If declined: drop GMO-served TLDs (.shop primarily, ~3% of daily volume and mostly low-quality candidates by post-filter score distribution) to `tlds.pending` and replace with other CZDS-approved TLDs — `.io` and `.co` are leading candidates given comparable apex-count scale and English-language defaults. Move to `pending`, don't delete, so the access stays alive for any future revisit.

### External-infrastructure failure modes — now daily

Both are external degradation, not bugs in our code. Both have been flagged for design attention but neither is blocking today's list.

- **web.archive.org connection-refused failures during the ~10:15–10:40 UTC window are a DAILY pattern.** Large fractions of enrichment candidates land as `wayback_unknown` (the three-state contract from 2026-05-17 keeps these as "unknown, don't penalize" rather than dropping them outright; the `cc_source_domain_count` exclusion in the scoring average is what keeps the score honest in this regime). The 10:15–10:40 window correlates with Internet Archive's daily maintenance / heavy crawl window; this isn't novel knowledge in the archival community, but it's now load-bearing for us because our enrichment runs in exactly that window. Possible mitigations under consideration: move Wayback enrichment off the daily hot path entirely (run it asynchronously and join later), or shift the cron further out. Both have downsides (asynchronous join complicates the publish flow; cron shift might collide with other registries' publish windows). Decision deferred.
- **crt.sh circuit-breaker trips most days on 502s.** Same class of issue — external service degradation. The `cert_history` field lands as `null` on those runs, scoring excludes it, and publication continues. No production impact beyond a missing column in the JSON contract for affected candidates.

### Paid-tier R&D (private repo)

Acknowledgement only. Strategy, moat definition, competitive analysis, engine internals, exact unit economics, and methodology all live in private artifacts and stay there. The test for each sentence in this subsection: "would this help a competitor if they read it?" If yes, it's not here.

- **R&D continues in a separate private repository (`domainsifter-engine`).** It is isolated from this repo's production pipeline — does not import from here, is not invoked by `pipeline.py`, touches no live data path. Status: progressing on its own track.
- **Competitive analysis exists privately.** Conclusions are not duplicated in this public file.
- **Paid-tier infrastructure direction mapped at a high level.** Concrete decisions and design details live with the private artifacts.

### Roadmap / phase gating

The load-bearing part. Read carefully.

- **NEXT after the next clean daily run: Phase 2 — LLM-based desirability ranker.** Haiku-tier model inserted between the lexical filter (`scripts/lexical_filter.py`) and the RDAP availability check (`scripts/pipeline.py:validate_availability`). Ranks post-lexical candidates by name-quality signal so the RDAP throttle budget is spent on the strongest candidates instead of the current `_bucket_and_cap_for_availability` trigram-prior trim. The ranker is **the cheap test** of whether the register-now drop pool warrants further paid-tier investment. It is fast to build, fast to validate, and answers the gating question.
- **Phase 2 gates further paid-tier work.** Downstream sequencing and component-by-component planning are kept in private artifacts; this public file says only that Phase 2 is the gate and nothing proceeds without its verdict.
- **Principle reaffirmed: prove the cheap thing before building the expensive thing around it.** Phase 2 is cheap, Phase 2 answers the gate, Phase 2 happens first.

### Calendar / pending decisions

- **~2026-06-06** — GMO Registry whitelist response decision point. If accepted: re-enable GMO RDAP bucket and remove the IP-block workaround logging. If declined: move GMO-served TLDs to `tlds.pending` per the fallback above.
- **~2026-07-15** — `.biz` CZDS reapply window. Administrative renewal, not first-time approval. Calendar item only; no code change anticipated.
- ~~**~2026-08-09** — GitHub PAT rotation.~~ **MISSED — caused a 9-day outage.** The PAT expired on schedule 2026-08-09; nobody rotated it. Every run 2026-08-10 → 2026-08-19 completed the pipeline then died at `git push` with exit 128 (`remote: Invalid username or token`), so `origin/main` froze on the 2026-08-09 data and the live site served stale listings for 9 days. The published count decayed 186 → 89 as each morning's `reset --hard` re-read the same stale carryover list. Resolved 2026-08-19: new fine-grained PAT `domainsifter-ovh-pipeline-2026-08` (Contents R/W + Metadata RO, 1-year expiry), plus the stranded 2026-08-18 commit `c8e69f2` rebased and pushed. **Lesson: a date written in STATE.md is not a reminder. The alerting worked — a ❌ FAILED email arrived every morning for nine days and went unread.**
- **2027-08-19** — **GitHub PAT expires** (`domainsifter-ovh-pipeline-2026-08`). Daily auto-commit push dies the next morning if missed. Put this in a real calendar, not this file.
- **~2027-09-17** — **Claude Code OAuth token expires** (`/etc/domainsifter/claude.env`, from `claude setup-token`, 1-year life, no auto-refresh). On expiry `claude -p` fails with `Login expired`; both LLM stages revert to their fallbacks. Regenerate on a machine with a browser and repaste. Same failure shape as the PAT above.
- **DMARC still at `p=none`** — was due for review per the 2026-04-30 follow-up note. No action taken; revisit during a deliverability-focused session.
- **Gmail spam flag on the daily operational report** — unresolved. Mark-not-spam workaround still in use. Sender is `hello@domainsifter.com` via Brevo SMTP; SPF/DKIM/DMARC all pass per the original validation; Gmail's heuristic is the unknown variable. Possible mitigation: add a domain-aged history to the sending pattern, or route operator reports via a dedicated subdomain. Deferred.

### What's NOT in this refresh (by design)

- Engine internals, methodology, datasets, cost/margin, and all paid-tier component design — **kept in private artifacts**, not duplicated here.
- Competitive analysis — kept private; conclusions not reproduced in this public file.
- Strategic claims about positioning, moat, or wedge — not stated here. Treat the public repo as an operational-and-pipeline record only.
...
## 2026-09-17 — Claude Code subscription backend staged (Phases 0–1 complete, pipeline UNCHANGED)

**Why this exists:** the Anthropic API credit balance hit zero on **2026-07-23** (partial failures from 2026-06-26). Both LLM stages have been dead since, failing soft and unnoticed for ~8 weeks:
- `phase2_ranker` — every batch 400s, falls back to mechanical trigram+length selection (`FALLBACK — too_few_eligible`, ~1,740 credit errors per run).
- `snapshot_classifier` — every domain returns `unknown` (e.g. `0 legitimate, 0 parked, 0 toxic, 33 unknown`). **The toxic-domain screen has been off since 2026-07-23.** Pre-outage it flagged 1–6 toxic domains on most days.

DomainSifter has no revenue and API credits will not be funded. Decision: drive both stages from Mario's **€120/mo Max subscription** via Claude Code (`claude -p` with `CLAUDE_CODE_OAUTH_TOKEN`) instead of metered API tokens. Routines were evaluated and rejected: 1-hour minimum interval, up-to-30-minute start jitter, and results only pushable to `claude/*` branches — a bad fit for two stages ~2h apart that need precise timing. Routine budget stays untouched at 15/day.

### What is installed and configured on OVH (2026-09-17)

- **`claude-code` 2.1.267** at `/usr/bin/claude`, installed from Anthropic's **signed apt repo** (`downloads.claude.ai/claude-code/apt/stable`). Key fingerprint verified as `31DDDE24DDFAB679F42D7BD2BAA929FF1A7ECACE` before trusting. Upgrades via normal `apt upgrade claude-code` — apt installs do NOT auto-update. No Node.js required (native binary); none is installed on the box.
- **`/etc/domainsifter/claude.env`** — mode `0640`, `root:domainsifter`. Holds `CLAUDE_CODE_OAUTH_TOKEN` (`sk-ant-oat0…`, 108 chars) generated by `claude setup-token` on Mario's laptop. **Valid ONE YEAR — expires ~2027-09-17.**
- **`/home/domainsifter/.claude`** — mode `0700`, owned by `domainsifter`, pre-created because the service account has `/usr/sbin/nologin`.

### Phase 1 validation results (all passed)

- The `domainsifter` service account runs `claude -p` fine despite `nologin`, via `sudo -u domainsifter -H bash`. No TTY needed.
- Authenticates on the **subscription** (`provider: firstParty`), not the API. **`ANTHROPIC_API_KEY` outranks `CLAUDE_CODE_OAUTH_TOKEN` in Claude Code's documented precedence — the pipeline's existing `.env` still contains it, so any invocation MUST `unset ANTHROPIC_API_KEY` or it will try to bill the empty API account.**
- `--model haiku` resolves to `claude-haiku-4-5`, matching the API-era model exactly.

### Calibration measurements (200 synthetic names, real `RANKER_SYSTEM` prompt)

| Measure | Value |
|---|---|
| Completeness | 200/200 returned, valid JSON, `reason` field intact |
| Output cost | **29.4 output tokens/name** → ceiling ~1,090 names per 32k-output call |
| Throughput | 52s per 200 names (~115 tok/s); output-bound |
| Per-call overhead | ~25k tokens of Claude Code system prompt + tools (cached across calls within the hour) |
| Thinking tokens | **0** with `MAX_THINKING_TOKENS=0` (defaults ON — 647 thinking tokens for a 3-name call before disabling) |

**Design consequences:**
1. **Chunk ~800 names per call** (safety margin under 1,090). ~16 calls for a 13,000-name day.
2. **~55 min sequential; ~15–20 min at 3–4 parallel.** The API path took ~10 min at concurrency 6. Pipeline has slack.
3. **The production system prompt is mandatory.** A loose paraphrase asking to score "registrability" was REFUSED outright — Claude read it as a trademark-law question and returned prose. Passing the real `RANKER_SYSTEM` via `--append-system-prompt` produced full compliance. **Prose-instead-of-JSON must be detected and treated as a failure that trips the existing mechanical fallback, never as an empty result.**

### Next: Phases 2–4 (NOT started — no pipeline file has been modified)

- **Phase 2 — toxic screen first** (smaller, and it is the safety check). New backend module behind a config switch `"backend": "api" | "claude_code" | "off"` per hard rule 9; API path retained intact for one-line rollback. **Design change needed:** the screen currently makes one call per domain (20–180/day); per-call overhead makes that wasteful, so group ~20 domains per call (~3–9 calls/day). Same verdicts, same output contract. Tests per hard rule 13. First live day in **shadow mode** — log verdicts, change nothing published, compare against the API-era July verdicts.
- **Phase 3 — ranker**, same backend, ~800-name chunks, existing fallback untouched. Shadow first.
- **Phase 4 — observability**: surface backend + fallback reason in the daily report so an 8-week silent degradation cannot recur; pin token expiry.

### Open risk (accepted by Mario, logged for honesty)

Anthropic's Claude Code docs state Pro/Max limits "assume ordinary, individual usage." A daily automated job feeding a commercial site is neither clearly inside nor clearly outside that. Volume is small (~1.2M Haiku tokens/day equivalent; the API path cost only ~$2.88/day). Risk if judged outside: account limiting — on the same account used for development. Mario has chosen to proceed.

### Usage-window note

First call lands ~09:32 UTC, opening a 5-hour Max usage window to ~14:32 UTC. The classifier at ~11:40 UTC falls inside the same window. Heavy work on the other project (`ismydomainok`) is best kept outside 12:30–17:30 Estonian time.

---

## 2026-09-18 — LLM stages restored on the Max subscription (Phases 2–4 built; SHADOW for first live run)

Continues the 2026-09-17 section. Phases 0–1 put `claude-code` and a subscription token on the box; this session built the code that uses them. **Full suite: 1110 passing.** Not yet deployed at time of writing — the server runs `git reset --hard origin/main`, so nothing takes effect until this lands on origin.

### The switch

`scripts/llm_backend.py` (NEW) is the single seam. `config.json` → `llm.backend` selects `"claude_code"` (subscription, free at point of use) or `"api"` (metered, the historical path, kept bit-for-bit for rollback). **Rollback is that one value.**

Two traps it exists to guard, both found empirically and both documented in its docstring:
1. **`ANTHROPIC_API_KEY` outranks `CLAUDE_CODE_OAUTH_TOKEN`** in Claude Code's auth precedence. The pipeline's `.env` still defines it, so the child env scrubs both it and `ANTHROPIC_AUTH_TOKEN`. Without that every call bills the empty API account and fails. A test asserts on the `env=` kwarg, and asserts the token never appears in `argv` (so it cannot leak to `ps` or journalctl).
2. **Claude Code is a conversational agent, not a completion endpoint.** A paraphrased prompt asking to score "registrability" was REFUSED as a trademark-law question. `parse_json_array` therefore treats prose as a hard failure so the caller trips its fallback, rather than recording an empty result. `MAX_THINKING_TOKENS=0` is set because extended thinking defaults ON and is pure waste here.

### Stage changes

- **`snapshot_classifier` — batched, v1 → v2.** Was one call per domain (`max_tokens=8`); Claude Code's ~25k-token per-call overhead makes that unusable, so it now sends `snapshot_classifier.batch_size` (20) domains per call and parses a JSON array. Failure containment is per-batch: a failed/refused/partial batch yields `unknown` for its domains only. Hallucinated domains not in the batch are dropped rather than applied to another record.
- **`phase2_ranker` — chunked at 800, 3-way parallel.** On `claude_code` the budget formula and the runtime cost meter are skipped entirely (no per-token cost); the cap comes from `phase2.max_names_claude_code`. **All budget behaviour is untouched on `api`.** New fallback reason `all_chunks_failed`. **Fallback parity preserved** — every fallback path still returns `lexical_kept` untouched so `_bucket_and_cap_for_availability` sees no `phase2_score` and runs its random-shuffle path; the pre-existing regression test passes unmodified, plus a new end-to-end companion.
- **`classify_carryover` — backend-aware, and it FORCES `shadow: false`.** Shadow is for the unattended daily run; this tool is invoked deliberately after a `--dry-run` review, so honouring shadow would make a wet run silently evict nothing — the exact repair it exists to perform. Gained `--config`. Its "no API key → abort" guard now means "no usable backend"; a broken binary or expired token fails per-batch into all-unknown instead.
- **`pipeline.py`** now threads `config` into `make_default_client()` and `classify_all()`. Without this, editing `config.json` would silently not reach the classifier at all.
- **`env_check`** no longer claims a missing `ANTHROPIC_API_KEY` disables classification — it only affects `llm.backend='api'` now, and the old wording would send an operator hunting the wrong thing mid-incident.

### Observability — the actual fix for the 8-week miss

`send_report` gained `LLM backend`, `Phase 2 ranker`, `Snapshot classes` and `Credit errors` fields, plus **subject-line escalation on an exit-0 run**:
- all-unknown classifier → `🚨 TOXIC SCREEN OFF` + a block spelling out that every published domain went out unscreened;
- ranker fallback → `🚨 PHASE 2 FALLBACK`;
- **shadow mode → `👁 SCREEN IN SHADOW`**, a deliberately *different* banner. Shadow genuinely leaves the gate open, so it must alarm — but an identical banner every day of a validation window is precisely how an operator learns to ignore it.

Also fixed an exit-0 hole predating this work: a lone surrogate in the journal raised `UnicodeEncodeError` out of `_truncate`, suppressing the email entirely. Encoding now uses `errors="replace"` — suppressing this report is the worst outcome, since it carries the alarms.

### Live rehearsal on OVH (2026-09-18, real backend, invented content)

Run as the `domainsifter` service account from a scratch dir; the deployed repo was not touched.

- **Classifier, 4 domains, ~2s:** pottery-studio page → `legitimate`; "Buy this domain" → `parked`; "Cheap Viagra, no prescription" → **`toxic`**; blank page → `empty`. All four correct.
- **Ranker, 30 names, 2 parallel chunks, 7.0s:** `batches_ok=2, batches_failed=0, cost_usd=0.0`, `MODE=RANKER`. 21 above the 60 gate, and exactly 9 below it — matching the 9 deliberately-bad names seeded (gibberish, spam-vertical, foreign-language, keyword-stuffed). Top picks: `housecalldoctor.com` 82, `familyhospital.com` 81, `dandeliontea.com` 79.

### Deployment state and what's next

- **SHADOW MODE IS ON** (`snapshot_classifier.shadow: true`). The first live run writes `snapshot_category_shadow`, leaves `snapshot_category="unknown"`, and **evicts nothing**. Compare the `SHADOW would evict N as toxic — <names>` log line against the API-era July verdicts, then set `shadow: false` to arm the gate.
- **Shadow verdicts are NOT persisted** — `output.py:_PERSISTED` and `domain_archive.py:_SIGNAL_FIELDS` are allowlists, so that log line is the only durable record. Add the field to an allowlist if a real A/B is wanted.
- **Backlog:** once armed, `python -m scripts.classify_carryover --only-unknown --dry-run` then without `--dry-run` cleans the ~8 weeks of unscreened entries in the published list.
- **Config-shape dependency:** the ranker treats a config with NO `llm` section as `api` (keeps every pre-existing test honest). Deleting the `llm` block would silently revert to the dead API path.
- **Observation, not fixed (out of scope):** `src/data/wayback_excerpts.json` is written by the pipeline but never `git add`ed by `run-daily.sh` Step 4, so each morning's `reset --hard` discards it. Worth a decision later.

---

## 2026-09-19 — Toxic gate armed, 22 spam domains evicted, content engine restarted

Second day on the subscription backend. Three things landed; the third is the one that matters commercially.

### 1. First live run of the new backend (09:00 UTC)

The ranker DROVE the published list for the first time since 2026-07-23 — `MODE=RANKER_PARTIAL`, 4,972 of 13,000 above the gate, 292 published. But it scored only **10,497 of 13,000**: two chunks of 800 died on malformed JSON (`Expecting ',' delimiter`) and four returned short (one gave 287 of 800). **2,503 names silently became below-gate without ever being judged.** Fixed same day (`930ae86`):

- `llm_backend.parse_json_array` salvages well-formed objects from a malformed or truncated reply instead of discarding the batch. The scanner respects string literals and escapes. **It does NOT weaken refusal detection** — prose contains no JSON objects, so a refusal still raises and still trips the fallback.
- `phase2_ranker` retries whatever is still unscored, at HALF chunk size. Retrying by NAME (not by chunk) covers hard failures and short returns together; in test, 3 of 5 names from a poisoned chunk were recovered.
- `phase2.chunk_size` 800 to **300**. Rehearsals at 15 and 200 names never exposed this; only production at 800 did.

### 2. The toxic gate is ARMED (`23a3a70`)

Shadow mode produced 29 verdicts and flagged 3 as toxic. All three verified by hand, all three correct, and **all three had been published to the live site**:

| domain | what the archived page actually was |
|---|---|
| `brightdawnnutrition.com` | Chinese gambling brand page, with "guaranteed win" copy |
| `languageidentifier.com` | Chinese adult content portal |
| `thegreenseason.org` | REAL African safari business, pages injected with Chinese lottery-gambling SEO |

**Not one would be caught by the keyword filter — every name is innocuous.** Only reading the archived CONTENT finds them. `snapshot_classifier.shadow` set to false.

### 3. Backlog cleanup — 22 evicted (`0838fcd`)

`classify_carryover --only-unknown` over all 289 screenable entries: 47 legitimate, 12 parked, **22 toxic**, 24 empty, 184 unknown (archive.org fetch failures). Published list 292 to **270**. Sidecar grew 303 to 535 excerpts.

Operator lesson: several evictions had entirely innocent names. `pentictonelvisfestival.com` (a real Elvis festival) served "Exchange Naughty Photos! - Festival Dating"; `allmysonsonbroadway.com` (a real play) served a Chinese porn portal; `oliviermartinez.net` served a Thai casino. **Expired domains get spam-squatted, and the name tells you nothing.** Claude's first instinct was to doubt these verdicts; the verdicts were right.

### 4. The content engine — why the site does not rank

Google Search Console, 2026-02-01 to 2026-09-18: **138 impressions, 8 clicks, average position 8.5.** Position 8.5 is not the problem. 138 impressions is. There is almost nothing indexed.

Cause: `archive_generator.py` writes the site's ONLY long-tail content. It produced 113 pages and stopped on **2026-07-21**, two days before the credit balance died. It has fired every day since via `OnSuccess=domainsifter-archive.service` — it is NOT unscheduled; that claim was made in-session and was wrong, and `STATE.md` already documented the `static` unit state as correct for an OnSuccess-triggered unit. Each day it tried 5 domains, took 5 credit-balance failures, tripped its breaker and exited non-zero. Silently, for two months.

Rebuilt this session:

- **On the subscription backend.** `ANTHROPIC_API_KEY` gate removed; `llm_backend` decides. Breaker threshold now config-driven.
- **Two prompt modes.** `GROUNDED` when a Wayback excerpt exists — the page states what the site WAS, citing title/meta/h1/h2 and the snapshot date. `NO-EXCERPT` — a SHORT factual page stating the content could not be retrieved. The old prompt speculated from the name ("the name suggests possible connections to fitness, productivity, entertainment, or educational content"); that phrasing is now explicitly banned. **Known residual:** the ban is not fully holding — a rehearsal page still produced "the domain's name suggests a connection to a filmmaker", though self-qualified in the next clause. Worth tightening.
- **Evidence persisted to frontmatter.** 7 new optional keys (`phase2_reason`, `snapshot_category`, `excerpt_title`, `excerpt_meta_description`, `excerpt_h1`, `excerpt_h2`, `excerpt_snapshot_timestamp`), all nullable+optional in `src/content/config.ts`. **Why:** the archive page is permanent but `daily-domains.json` and the sidecar are 14-day rolling windows — zero of the 113 existing archive entries still appear in the daily list, so evidence read from those files would vanish on a later rebuild. Untrusted text is control/bidi-stripped and YAML-quoted; a hostile fixture (embedded quotes, colons, a literal triple-dash, a planted `name:` line) is in the tests.
- **`src/pages/d/[domain].astro`** renders "Why we shortlisted it" and a "What this site used to be" evidence block, frontmatter-first with the rolling files as fallback. All excerpt text is `{}`-interpolated (never `set:html`), control/bidi-stripped, length-capped, `dir="auto"`, and outbound links are allowlisted to web.archive.org with `rel="nofollow ugc noopener noreferrer"`.
- **`run-daily.sh` now commits `src/data/wayback_excerpts.json`.** It was written every run and deleted every morning by Step 1's `reset --hard`, so only `classify_carryover` ever persisted it.
- **`systemd/domainsifter-archive.timer`** (14:00 UTC) as a SAFETY NET — `OnSuccess=` only chains after a successful pipeline, so a failed-pipeline day never archives at all. NOT YET INSTALLED on the server; see operator step below.

### Rehearsal evidence (live on the box, before deploy)

`legendarybusinesses.com`, generated through the real backend:

> "legendarybusinesses.com operated as a celebrity wealth and career reference site... The h2 headings visible in the June 14, 2026 snapshot show article headlines covering specific public figures: Danae Hays, David Faustino, Hakimi, Natanael Cano, and Brandon Frankel."

A parked page (`allancarrmovie.com`) correctly reported the NameBright placeholder as the finding and stated the snapshot does not reveal what the site was during its active years.

### OPERATOR STEP — timer not installed

```
cd /home/domainsifter/domainsifter && sudo -u domainsifter git pull --ff-only
sudo cp systemd/domainsifter-archive.timer /etc/systemd/system/
sudo systemctl daemon-reload && sudo systemctl enable --now domainsifter-archive.timer
systemctl list-timers | grep domainsifter
```

### Contract change (hard rule 10)

`daily-domains.json` gained `phase2_reason` (string or null) — additive, non-breaking. Null on fallback days, on pre-migration carryover, and for the ranker's "missing from response" placeholder, which `output._phase2_reason` maps to null so it never reaches the site.

### Open decisions

- **Regenerating the 113 old pages** under the grounded prompt. Deliberately NOT done: the idempotency guard skips any domain with an existing `.md`, so it needs an explicit delete of those files plus their index entries. Most of those domains are registered by now, and many have no retrievable excerpt, so they would shrink to short factual pages. Look at real grounded output for a few days first.
- **Archive volume.** Only `Clean`/`Promising` qualify — 1 to 3 pages/day against a ~270-domain list. That threshold is the main lever on content growth, and the reason only 113 pages accumulated in three months.
- **`bettingmastertechnic.com` is on the published list.** "betting" is in `rejected_keywords`, but that list is token-exact; the apex is a single token so it never matches, and `rejected_keyword_substrings` does not include it. A filter gap worth reviewing.

---

## 2026-09-20 — First clean run on the new stack; toxic verdicts made permanent

### The run worked

| | 2026-09-19 | 2026-09-20 |
|---|---|---|
| Names scored | 10,497 / 13,000 | **12,997 / 13,000** |
| Chunks failed | 2 | **0** |
| Ranker mode | `RANKER_PARTIAL` | **`RANKER`** |
| Archive pages | 0 (dead since 07-21) | **+14** (113 → 127) |

The retry pass recovered 286 of 289 unscored names. `snapshot_toxic: 1` — the gate evicted at publish time for the first time. Published 259. The newsletter credibility line went live with the real figure: **"Scanned 179,215 dropped domains today; 2,466 passed our filters into per-domain checks; 259 made the published list."**

### The defect this run exposed — and the fix

`unknown` is BOTH "not abusive" AND "the archive.org fetch failed", and verdicts were recomputed from scratch every run with nothing remembered:

- **2026-09-19** `ridgemotorsports.net` fetched fine → classified **toxic**.
- **2026-09-20** fetch failed → back to `unknown` → passed the gate → stayed published → **given a permanent archive page**.
- Re-verified by hand: its archived page is "London Slot 69 : Dari Kecepatan Balap ke Sensasi Slot Online" — Indonesian online-slot gambling. `fithealthyplanet.com` and `wyevalleyhistory.net` took the same path.

All three evicted and their pages deleted (`eecf865`). The fix (`7e4f6ea`): **`scripts/toxic_denylist.py`** — append-only record in private R2 (`state/toxic_denylist.jsonl`) of every domain ever classified toxic. Never aged out; never committed to the public repo (it is a list of named domains we judge abusive). Fails soft — R2 down degrades the gate to live-check-only and logs loudly, never blocks a run.

Wired at four sites: `pipeline.py` (records after the sidecar write, then consults in the post-enrichment filter), `classify_carryover.split_toxic` (the function whose correct verdict was forgotten — now both records and consults), `archive_generator._filter_qualifying` (refuses to mint a page for a remembered domain, because a page gets indexed and is far harder to retract than a list entry), and `send_report` (surfaces "N by today's check, M from memory"). `filter.py` uses a DISTINCT reason `snapshot_toxic_remembered` so that second number means exactly "times memory caught what the live check missed".

**Seeded with 6 hand-verified domains** on 2026-09-20: brightdawnnutrition.com, languageidentifier.com, thegreenseason.org, ridgemotorsports.net, fithealthyplanet.com, wyevalleyhistory.net.

### Known limitation, now the binding constraint

**All 14 new archive pages came out in NO-EXCERPT mode** — archive.org served no content for any of them. The pages are the short, honest variant rather than the grounded pages rehearsed on 2026-09-19. Archive.org's reliability now caps BOTH the content quality of new pages AND the toxic screen's coverage (184 of 289 fetches failed on the 09-19 sweep). This is the highest-value thing to work on next: consider retry/backoff, a second capture source, or caching successful excerpts more aggressively.

### Open items for the next session

1. ~~**Common Crawl is 4 months stale and was NEVER automated.**~~ **RESOLVED later the same day — see the "Common Crawl refresh automated" section at the end of this file.** Note for the record that the claim "CC publishes the domain-level hyperlink graph ~3x/year" in this item was **WRONG**: CC publishes MONTHLY as a rolling 3-month window, and `STRATEGIC_NOTES.md` had that right since 2026-05-13. The corrected cadence is what drove the weekly (not monthly) timer.
2. **The newsletter has still never been sent.** 7 organic subscribers, drafts accumulating since May (128 as of 09-19, all `draft`). Today's draft is clean and verified. Mario sends manually; a re-introduction note in his own voice should precede the first daily issue.
3. **Auto-send** is easy technically (`status: about_to_send` via API) but should stay manual until several good issues have shipped, then be gated: only auto-send if the ranker drove, the toxic screen ran, and N picks carry reasons.
4. **Buttondown has no caller-supplied plain-text field.** Confirmed against the live API: the email object exposes only `body`. `newsletter.plaintext_api_field` should stay empty permanently; the built text part is unused. Buttondown derives its own.
5. **Verdict distribution.** 250 Caution / 20 Promising / 0 Clean on 09-19. `Clean` needs score ≥70 and the daily top score is ~66, so it is effectively unreachable. This gates archive-page volume too (only Clean/Promising qualify — ~20 of 259). Deciding what Clean should mean is the main lever on content growth.
6. **The 113 pre-2026-09-19 archive pages** still carry speculative prose and are NOT regenerated (idempotency guard skips any domain with an existing `.md`). Regenerating needs an explicit delete of those files plus their index entries.
7. ~~**Newsletter intro text names the CC release**~~ **RESOLVED the same day, structurally rather than by editing the string**: `intro_text` now carries a `{cc_release}` placeholder resolved at render time from `cc_backlinks.latest_release`, so it cannot drift again — which matters because the release now changes monthly. `DEFAULT_INTRO` in `generate_newsletter.py` was found to still contain the exact "last 3 months of crawl data" claim that `7755c61` removed from config only, and was de-staled too. Today's already-generated draft still holds the old literal wording; it is a draft and was never sent, so it can simply be regenerated or discarded.

### Test surface

1395 tests passing. Suite runtime is now ~187s — worth watching.
---

## 2026-09-20 (later) — Common Crawl refresh automated

### What was actually wrong

`cc_source_domain_count` carries scoring weight **0.30 — equal to `wayback_snapshots`**. `cc_refresh.py` had no systemd timer and no cron entry and had run **exactly once, by hand, on 2026-05-13**. Four releases went by unnoticed (mar-apr-may, apr-may-jun, may-jun-jul, jun-jul-aug). Nothing alarmed, because a stale-but-present SQLite returns plausible numbers — the same shape of failure as the 2026-07-23 → 2026-09-17 LLM outage.

The 2026-05-13 strategic note had predicted the exact failure mode ("if Mario is on vacation when CC publishes, we miss a release") and set a revisit date of "July-August 2026", which itself passed unobserved. Both Common Crawl open questions are now closed in `STRATEGIC_NOTES.md`.

### The factual correction that shaped the design

**Common Crawl publishes the domain-level hyperlink graph MONTHLY, as a rolling 3-month window — not ~3x/year.** Verified against `commoncrawl.org/web-graphs` and by HEAD-probing the artifacts: all six 2026 windows exist (`jan-feb-mar` … `jun-jul-aug`), and `jul-aug-sep` 404s as of 2026-09-20, so a release lands during the last month of its own window (`jun-jul-aug` was published 2026-08-25).

`STRATEGIC_NOTES.md` recorded "roughly monthly" correctly back in May. The "~3x/year" belief lived only in this file, and it mattered: a monthly timer would otherwise have been the obvious cadence, and a monthly timer that gets killed or fails leaves a 0.30-weight input stale for a month.

### Cadence: weekly, and why

`OnCalendar=Sun *-*-* 18:00:00 UTC`. Discovery is two HEAD requests and exits 0 as a no-op when the newest release is already installed, so a weekly tick is free. The real reason for weekly over monthly is **retry**: any failed or runtime-capped run is automatically re-attempted in 7 days. The latency cost is irrelevant — the data is a 3-month-wide window and the staleness alarm is set at 45 days.

18:00 UTC sits 3.5 h past the archive timer's worst case (~14:30) and 5.25 h past the daily chain's last push (~12:45). Both matter because `run-daily.sh`, `archive_generator.py` and now this job all `git push` to origin/main, and a push landing while origin has moved is how this project broke on 2026-05-21.

### The three safety properties

1. **The config swap is gated on in-R2 verification.** `verify_derived_release` deletes any cached copy, re-downloads the derived SQLite **from R2** (not from the local build), opens it read-only and checks `meta.release`, `COUNT(*) >= min_cc_apex_rows`, every `canary_present` apex above its floor, and every `canary_absent` apex genuinely absent. Only then is `cc_backlinks.latest_release` rewritten. A failure leaves the previous release installed and the pipeline scoring on known-good data — **degraded, never half-swapped mid-run** — and the bad cache file is deleted so it can never be served. The round-trip doubles as a **cache pre-warm**, so the 09:00 run never pays a 6.4 GB inline download during enrichment.
   Also relevant: `pipeline.py` reads config exactly once, at startup, so a swap cannot half-apply *within* a run. The exposure was only ever the git push window.
2. **The job cannot still be running at 09:00 UTC.** `TimeoutStartSec=10h`: Sun 18:00 + 10h = Mon 04:00 kill at the latest, **5 hours clearance**. Enforced by PID 1, not by hoping the schedule is early enough. **A trap worth recording: `RuntimeMaxSec=` has no effect on `Type=oneshot` units** (the whole `ExecStart` runs during the start phase, so the unit never reaches "running"), and `TimeoutStartSec=` defaults to **infinity** for oneshot rather than to `DefaultTimeoutStartSec`. The intuitive setting would have produced no cap at all and nothing would have warned. A kill is safe: every step is idempotent and the download resumes by HTTP Range.
3. **`Persistent=` is deliberately absent** — it is set on `domainsifter.timer` and `domainsifter-archive.timer`; this unit differs on purpose, do not "fix" it. A boot catch-up replays at an arbitrary time of day, including mid-pipeline, and would start a multi-hour disk- and network-heavy job that pushes to origin/main inside the daily run's push window. A weekly cadence makes catch-up pointless anyway.

Two further interlocks, both config-driven under `cc_backlinks.refresh`: a **blackout window** `[07:00, 16:00)` UTC and a check that `domainsifter.service` is not active. Either one makes `--auto` log a warning, write `action: "skipped"` and **exit 0** — a deliberate skip must not page anyone, and a persistently-skipping timer is caught by the staleness line in the daily email instead.

### Git discipline in the wrapper

`scripts/run-cc-refresh.sh` mirrors `run-daily.sh`: `set -euo pipefail`, `fetch` + `reset --hard origin/main` first so the config edit lands on top of current origin, `GITHUB_TOKEN` only ever in the `git push` argv, and **no `set -x`** (it would leak the token into journalctl).

It touches git **only** when `action == "installed"`. `noop`, `skipped`, `verification_failed`, `discovery_failed`, an unknown action, a missing result file and an unparseable result file all leave git completely alone — pushing on `verification_failed` would publish a config pointing at an artifact that failed its own canaries. The result JSON is parsed with the venv python, never grep/sed.

Two non-cosmetic details found while testing it:

- The result fields are joined with **US (0x1f), not tab**. Tab is IFS whitespace and bash `read` collapses runs of it, so a null `previous_release` silently shifted `rows` into `PREVIOUS` and `reason` into `ROWS`. This was a real bug, caught in a sandbox run.
- The gitignored result file is `rm -f`'d **before** the run, so a crash that writes nothing can never be misread as last week's `installed`.

Push retries 3x with `fetch` + `rebase` between attempts, never force. Every exit path logs one line, so last week's decision is one grep away:

```
journalctl -u domainsifter-cc-refresh.service | grep 'cc-refresh summary'
```

### Anti-recurrence: the daily email

This is the part that stops it happening again. `send_report.py` adds a line of the form:

```
CC backlink data : cc-main-2026-jun-jul-aug, built 0d ago, last refresh: installed
```

Age comes from `meta.built_at` in the **locally cached** SQLite (opened read-only; it never downloads 6.4 GB to build an email), falling back to `finished_at` in the refresh result file, falling back to an explicit "unknown" — never guessed, and "unknown" never escalates. The subject escalates on **🚨 CC DATA STALE** (age > `staleness_warn_days`, 45) or **🚨 CC REFRESH FAILED** (last action `verification_failed` / `discovery_failed`), with a body banner in the existing style. Fails soft at every source: missing config, missing cache, corrupt SQLite, malformed result JSON and unparseable timestamps all degrade to a readable "unknown" line and never block the email.

### Retention decision: prune raw, never delete derived

Supersedes the "we never delete old releases" line from 2026-05-13 — refined, not reversed, because that note never distinguished the two artifact classes:

- **Derived SQLite** (~6.4 GB/release) — **never deleted.** It is what the decay-velocity thesis in `STRATEGIC_NOTES.md` actually queries, and the only artifact the pipeline touches.
- **Raw vertices+edges** (~10–19 GB/release) — **pruned to the newest 2 releases.** Raw exists solely to build derived. The original "you can't go back in time" argument does not apply to it: data.commoncrawl.org keeps its hyperlinkgraph releases indefinitely and serves them free (verified — the whole 2025 back-catalogue still returns 200). The newest 2 are kept so a discovered build bug can be re-derived without a re-download.

Cost: raw pinned at ~21 GB IA (~$0.21/mo) instead of growing every month; 5-year projection ~$14/mo → ~$6/mo with **zero** loss of capability. Three independent guards: the pruner refuses to delete anything under `cc/derived/` at any config value, refuses to touch the active release, and skips any release name it cannot parse. `prune_raw_after_releases: 0` disables it. The local `~/.cache/domainsifter/cc/` is pruned to the active release too — at 6.4 GB per monthly release an unpruned cache fills the box.

### The disk pre-flight bug

`_ensure_disk_space(workdir, required_bytes=25 * 1024 * 1024 * 1024)` was a hardcoded magic number (rule 9) and **badly undersized**. Measured on the box during this build, filesystem `used`:

| stage | used |
|---|---|
| before the run | 11 GB |
| raw downloaded (10.3 GB), uploading | 20 GB |
| **DuckDB edge aggregation** | **49 GB** |
| aggregation done, spill released | 25 GB |
| writing the 6.4 GB derived SQLite | 29 GB |
| complete | 26.8 GB |

So ~38 GB peak against 10.3 GB of raw — a **~3.7x multiplier** — versus a check demanding 25 GiB. On any volume between those two numbers the check passes and the build then dies on ENOSPC deep into a multi-hour job.

Now computed as `(vertices + edges Content-Length) x cc_backlinks.refresh.disk_headroom_multiplier`, default **5.0** — not 3.7, because the sampling was coarse (5-minute intervals over a 5m33s aggregate step, so the true peak is likely higher) and because the spill scales with the edges file's **row** count rather than its compressed size. Falls back to a 60 GiB floor if HEAD gives no sizes; never skips the check. Applies to `--build-only` too, which also needs the spill room.

**Do not "fix" `_DUCKDB_MEMORY_LIMIT = "8GB"`.** It is deliberately conservative so behaviour matches a 16 GB dev laptop, and the spill is a chosen disk-for-portability trade, not a defect — it does not shrink on the 125 GB box.

### Signal quality: the edge set is shrinking, unevenly

Upstream `edges.txt.gz` by release: 17.2 (jan-feb-mar), 18.2 (feb-mar-apr), 16.5, 14.6, 10.5, **9.4 GB** (jun-jul-aug). Vertices are stable at 0.88–0.92 GB. The naive expectation — counts roughly halve, scores drop ~2 points — **is wrong**:

| apex | feb-mar-apr | jun-jul-aug | ratio |
|---|---|---|---|
| google.com | 16,365,926 | 16,347,517 | 1.00x |
| github.com | 747,095 | 752,451 | 1.01x |
| wikipedia.org | 2,142,147 | 2,147,162 | 1.00x |

But the distribution shows where the loss actually landed:

| bucket | =0 | 1–9 | 10–99 | 100–999 | >=1000 |
|---|---|---|---|---|---|
| feb-mar-apr | 16,875,725 | 95,556,571 | 9,746,018 | 1,626,233 | 842,163 |
| jun-jul-aug | 14,903,407 | 93,622,024 | 9,594,383 | 1,380,112 | 222,959 |
| ratio | 0.883 | 0.980 | 0.984 | 0.849 | **0.265** |

Mega-sites are saturated and unaffected; the 0–99 buckets where dropped domains overwhelmingly live are ~0.98, so **day-to-day scoring barely moves**. The collapse is concentrated in the >=1000 population — exactly the high-authority tail that would mark a genuinely valuable find. No weights were retuned on a single observation (rules 5 and 9); this is logged as something to watch, and it strengthens the case for the multi-release decay strategy, which needs only the derived SQLites we are keeping.

### Surface

New: `systemd/domainsifter-cc-refresh.service`, `systemd/domainsifter-cc-refresh.timer`, `scripts/run-cc-refresh.sh`.

Modified: `scripts/cc_refresh.py` (release-name algebra, discovery, guards, in-R2 verification, install, prune, disk sizing, `--auto` / `--install` / `--discover-only`), `scripts/config.json` (`cc_backlinks.refresh` block), `scripts/generate_newsletter.py`, `scripts/send_report.py`, `STRATEGIC_NOTES.md`, `RECOVERY.md` (new "Weekly Common Crawl refresh" scenario).

Untouched: every site component (no `.astro` file hardcodes a release — checked), the output JSON contract, and the pipeline's scoring and RDAP paths. `Methodology.astro`'s "released monthly, covering a rolling 3-month window" turns out to have been **correct** all along.

`scripts/deploy_systemd.sh` needed no edit — it globs `systemd/*.service` and `systemd/*.timer`.

**1557 tests passing (+162 over the 1395 baseline), suite ~198s.**

### The live rollout, in order

Everything below actually ran on the box today; none of it is projected.

1. **Manual refresh of the new release** (transient systemd unit, production User/EnvironmentFile semantics), 15:00:19 → 15:21:09 UTC, **20m50s**, exit 0. Download 5m47s for both files at ~28 MB/s, edges aggregated in 333.1s, full build 612.8s, derived upload ~1m50s.
2. **Units deployed** via `sudo scripts/deploy_systemd.sh` — it reported `updated: domainsifter-cc-refresh.service`, `updated: domainsifter-cc-refresh.timer`, one `daemon-reload`, everything else unchanged. No edit to the deploy script was needed.
3. **The blackout interlock was tested live before trusting it.** Started the service at 15:31 UTC, inside `[07:00, 16:00)`:
   `action=skipped ... reason=inside the blackout window [07:00, 16:00) UTC`, exit 0, `Result=success`, and `git rev-parse HEAD` unchanged before and after. A guard that has never been observed refusing is not a guard.
4. **The install ran through the real automation**, not by hand, so the gated path got exercised for real. At 16:01 UTC: `Verification PASSED for cc-main-2026-jun-jul-aug: 119722885 rows, canaries {'google.com': 16347517}` → config swapped → committed `2e0635f` → pushed to origin/main. `action=installed previous=cc-main-2026-feb-mar-apr pushed=yes exit=0`. Whole thing took 55 seconds, because raw and derived were already in R2 and the idempotency checks skipped them; the only real work was the 6.4 GB verification download from R2 at ~128 MB/s.
5. **The verification download left the pipeline cache pre-warmed and pruned**: `~/.cache/domainsifter/cc/` went from holding only `feb-mar-apr` to holding only `jun-jul-aug`. Tomorrow's 09:00 run pays no inline download.
6. **The disk fix was confirmed in production**, not just in tests:
   `Disk requirement for cc-main-2026-mar-apr-may: raw 16.2 GiB (vertices 0.8 + edges 15.4) x 5.0 headroom = 81.2 GiB free needed`
   — 81 GiB demanded where the old hardcoded check asked for 25.

### Backfill of the four stale months

**Nothing had been permanently lost.** Common Crawl keeps its hyperlinkgraph releases indefinitely, and all three missed releases were still served. Confirmed by HEAD probe on 2026-09-20, along with `2025-oct-nov-dec`, `2025-jan-feb-mar` and `2024-may-jun-jul` (all 200). 2020/2017 404 under this URL pattern, but CC's index lists those years, so their naming convention simply differs — worth knowing before anyone tries to backfill deeper with `_release_name`.

Backfilled sequentially 16:02 → 17:32 UTC (**90 min**, `rc=0`) via the plain `--release X` path. **Deliberately without `--install`** — `--install` would have run verify+swap and pointed `latest_release` at an *older* release than the one just installed, silently downgrading the 0.30-weight input. The workdir was deleted between releases so peak disk stayed at one release's footprint instead of three.

| release | rows | derived | google.com |
|---|---|---|---|
| `cc-main-2026-feb-mar-apr` | 124,646,710 | 6.15 GiB | 16,365,926 |
| `cc-main-2026-mar-apr-may` | 118,760,321 | 5.87 GiB | 15,679,130 |
| `cc-main-2026-apr-may-jun` | 121,091,933 | 6.00 GiB | 16,584,611 |
| `cc-main-2026-may-jun-jul` | 117,963,409 | 5.85 GiB | 16,387,753 |
| `cc-main-2026-jun-jul-aug` | 119,722,885 | 5.94 GiB | 16,347,517 |

**Unbroken monthly coverage Feb–Aug 2026.** Note how stable google.com is across all five (15.7M–16.6M) even though upstream edges halved over the same span — see the signal-quality section above for where the loss actually landed.

The backfill path does **not** verify, so those three artifacts were unchecked. Raw is the only cheap rebuild path for an unverified derived SQLite, so nothing was pruned until all three were run through `verify_derived_release` against R2 — all three PASSED. Only then:

- **Local cache** pruned back to the active release.
- **Raw** pruned for `feb-mar-apr`, `mar-apr-may`, `apr-may-jun`; kept for the newest two (`may-jun-jul`, `jun-jul-aug`). The log line to look for is `Raw retention: keeping [...]; candidates for deletion [...]`.
- **All five derived SQLites retained** — the pruner never touched `cc/derived/`, which is the property that matters most.

Final R2 state: **50.02 GiB** under `cc/` — 4 raw objects on `STANDARD_IA` (20.2 GiB) and 5 derived on `STANDARD` (29.8 GiB). Steady state from here is ~21 GiB of raw pinned plus ~6 GiB of derived added per month.

### What this does and does not give us

Worth being precise, because "we now accumulate releases" invites a wrong conclusion:

- ✅ Effectively a **monthly pull** (weekly check, installs when CC publishes).
- ✅ Derived SQLites **accumulate and are never deleted** — a growing archive of monthly snapshots.
- ❌ Raw does **not** accumulate; capped at the newest 2 on purpose.
- ❌ **Nothing queries across releases.** `cc_backlinks` reads only `latest_release` (Strategy A). We hold an archive of snapshots, **not** a per-domain history database. The decay-velocity capability is *enabled* by keeping the snapshots and is *not built*.

The design for closing that gap — and why the obvious "merge everything into one table" version is the expensive wrong answer at 120M rows x N releases — is written up in `STRATEGIC_NOTES.md` under "Common Crawl backlink history — how to actually get it". Planned, deliberately not built. The key point recorded there: forward history is nearly free (record what `enrich()` already fetches, ~2,500 rows/day, `domain_archive.py` idiom) and compounds from the day it is switched on, while backward history is an offline batch job over the snapshots we are now keeping.

**One trap documented there that matters for any future decay maths**: monthly releases are rolling 3-month windows, so consecutive releases share **two of their three months**. Adjacent releases are not independent observations. Use samples three or more releases apart for a slope; adjacent ones only for smoothing.

### Still open

- `CC_BACKLINKS_RELEASE` env var takes precedence over config in `cc_backlinks._resolve_release`, but the newsletter intro reads config only. If that var were ever set on the box, the enricher would query one release while the newsletter named another. **Verified not set anywhere** (`.env`, unit files, `/etc/environment`) — flagged, not changed, because changing it would alter a documented contract.
- The blackout guard only evaluates at start, so it cannot stop a job already running when 07:00 arrives. The 10h cap is the only thing enforcing that, which is why it is not optional.
- A rolled-back release will be re-installed by the next Sunday tick. The rollback recipe in RECOVERY.md therefore tells the operator to also set `cc_backlinks.refresh.enabled: false` and record why.
- Today's already-generated newsletter draft still contains the old literal release string. It is a draft and was never sent; regenerate or discard.
---

## 2026-09-20 (later still) — the archive is now actually used, not just stored

### Why this followed immediately

Automating the refresh and backfilling the gap left R2 holding five monthly derived SQLites while the pipeline read exactly one of them. The owner's objection, verbatim in substance: *the idea was we collect the data so we can always offer more info pulled straight from our archive — if we use only one part of it how does that make sense?* Correct. Strategy A was chosen on 2026-05-13 when there **was** only one release; with five it is just a cache with extra steps.

### Scope: display only, and that boundary is load-bearing

`cc_source_domain_count` keeps its exact prior meaning (the value from `latest_release`) and its **0.30** scoring weight. The new `cc_backlink_history` field feeds **no filter, no score and no verdict**. This change is structurally incapable of moving the published list or its ordering, and that is pinned by tests rather than asserted:

- A parametrized regression test projects a row **with** a history (good, empty, null, and 13 malformed shapes) and asserts it is dict-equal to the same row **without** one on every other key, including `score` and `verdict`.
- A Promising-boundary test where a 99,999-strong history must **not** rescue a `Caution` verdict.
- An ordering test where the lowest-scoring domain is given the richest history and the published order does not move.
- Independently verified that nothing in `filter.py` / `score.py` / `output.py` iterates a candidate dict generically (`grep` for `.items()` / `.keys()`), so a new key cannot be swept up implicitly. `score_candidate` builds a hardcoded 5-tuple from named `.get()`s.

Scoring on backlink decay remains a separate, later decision that wants observed history to calibrate against — the `STRATEGIC_NOTES.md` principle of proving the cheap thing first.

### How the window is chosen

**Auto-discovered**, deliberately: `cc/derived/` is listed in R2, names are parsed and sorted **chronologically** (reusing `cc_refresh._parse_release` / `_release_sort_key`), `latest_release` is forced to the head, and the list is truncated to `cc_backlinks.history.max_releases` (default 6). There is no hand-maintained release list anywhere, because a manual list is precisely what went four months un-updated and caused this entire work item. A newly installed release joins the window on its own.

Chronological sorting is not a detail: lexicographically, `apr-may-jun` sorts before `feb-mar-apr` before `jun-jul-aug`, which would scramble the series. Verified against live R2 — the box logged the window as `['jun-jul-aug', 'may-jun-jul', 'apr-may-jun', 'mar-apr-may', 'feb-mar-apr']`.

### Two latent bugs this surfaced, both config-dependent and silent

1. **`prune_local_cache` kept only the active release.** With a window that means every install deletes the archive cache, and the next daily run re-downloads ~30 GB **inline during enrichment**. It now keeps `keep_release` ∪ the window, and — the important part — **keeps everything when the window cannot be determined**. A wrong deletion costs a 30 GB download inside the 09:00 run; a skipped deletion costs some disk. There is an extra guard beyond the obvious one: `history_window_releases` is itself fail-soft and returns `[latest_release]` on an R2 listing failure, which is **byte-identical to a healthy one-release window**, so a single-entry window is treated as unknown whenever `max_releases > 1`. Without that, one transient `ListObjectsV2` hiccup would still have wiped the cache.
2. **The in-memory config went stale across the swap.** `install_release` rewrites `latest_release` on disk while the dict this process holds still carries the old value — and the window is both *keyed on* and *headed by* that value. At the default cap of 6 the window holds everything either way, so it is invisible; at a small cap it would pre-warm the superseded release and hand the pruner a keep-set protecting it while allowing deletion of one the window wants. `_sync_installed_release` fixes the dict **and then** drops the cache; the reset alone is insufficient, because a recomputed window is still recomputed under the same stale head. Pinned by a test at `max_releases: 2` — the only cap where right and wrong differ, and it was confirmed to fail when the bug is reintroduced.

### Pre-warming

Every window release must be cached **before** the 09:00 UTC pipeline needs it, or the first run after a new release pays ~6 GB inline mid-enrichment (~30 GB for a cold window). The window is pre-warmed right after a successful install, gated on `history.prewarm_on_refresh`, in the order **verify → install → resync+reset → pre-warm → prune raw → prune local** so the prune sees the files the window wants instead of racing the download. Pre-warm is non-fatal: by then the release is verified and installed, and a cold cache only costs time on the next run.

New CLI: `python -m scripts.cc_refresh --prewarm-history` caches the window and exits — no discovery, no build, no config write, no git. Mutually exclusive with the other mode flags.

### Measured on the box

Pre-warm of the 4 uncached releases: **2 minutes**, ~28 s per 6 GB release (~215 MB/s from R2), `5 of 5 releases cached`, exit 0. Disk 50 GB used of 467 GB, 394 GB free.

Live enrichment against the real cache:

| apex | newest → oldest |
|---|---|
| github.com | 752,451 · 748,920 · 751,171 · 750,824 · 747,095 |
| wikipedia.org | 2,147,162 · 2,154,598 · 2,180,577 · 2,155,083 · 2,142,147 |
| marketglow.com (invented) | field absent — not in any release |

`history[0]["source_domain_count"] == cc_source_domain_count` by construction (entry 0 reuses the already-fetched count). Cost is negligible: first call 440 ms to open five SQLite connections, then **200 candidates x 5 releases in 20 ms total — 0.10 ms per candidate**, so a 2,500-candidate day adds roughly a quarter of a second.

### Disk, and the lever

~6 GB per cached release. `max_releases: 6` is ~36 GB against a 467 GB volume, and the window grows ~6 GB/month until it reaches the cap. **That cap is the single lever** — raise it deliberately after checking free space. `history.enabled: false` collapses retention back to active-release-only, so disabling the feature also reclaims the disk.

### Site

`src/components/DomainTable.astro` — a hand-rolled inline SVG sparkline in the existing Backlinks column and in the mobile cards. No library, no new npm dependency, no new JS (the existing `<script>` block is untouched), so there is no new client-side failure path.

**This touched a file hard rule 19 forbids during pipeline work.** The owner explicitly authorised a site change for this feature, choosing "in the JSON + on the site" over a backend-only option; the waiver is scoped to this one file and the diff extends the existing Backlinks cell rather than restructuring anything.

Honesty constraints built into the rendering:

- **`null` is not `0`.** A null (apex not in that release's graph) breaks the polyline and draws no point, with x-positions keyed to the index in the full series so the gap stays visible rather than the timeline silently compressing. Nothing is interpolated across a null (hard rule 2). Tooltip distinguishes `0 source domains` from `not in this release`.
- **No trend percentage is claimed.** Consecutive releases are rolling 3-month windows sharing two of three months, so adjacent points are heavily autocorrelated — at five releases there are only about two independent observations. Only a coarse direction past a ±20% end-to-end threshold (`↗ rising` / `↘ falling` / `→ holding steady`), with the overlap caveat in both the `title` and the screen-reader text.
- **Fewer than two known values renders nothing** — one point is not a trend, and the Backlinks number already says everything known.
- Accessibility: the SVG and arrow are `aria-hidden` with an `sr-only` sibling carrying every release, every count and the direction.

**Absence is provably invisible.** The component was built at `HEAD` and with the change against the same history-free production data, and the rendered `dist/index.html` was diffed: **zero differences**. So the live site is unchanged for every row lacking the field — which is every row until the next pipeline run, plus all pre-existing carryover, plus the sample-data fallback.

### Contract

`cc_backlink_history` is documented in **both** `PLAN.md` Principle 5 and the `output.py` migration list (hard rule 10): append-only, optional, `null` never fabricated. Validation is **all-or-nothing** — any bad entry drops the whole field to `null` with one warning per candidate, and since only `str`/`int`/`None` are emitted the field is structurally incapable of making `json.dump` fail (rule 17: invalid JSON output is crash-worthy, so the correct degradation is valid JSON with the field nulled).

**Carryover needed no change**, verified rather than assumed: `carryover.load_existing` returns published rows whole with no field whitelist, and every transform copies via `{**e, ...}`. A row written before today simply lacks the key and is published as `null`.

**1677 tests passing** (+282 over the 1395 baseline at the start of the day). Site builds, 136 pages.

### What is still not built

Backward history for a domain we have never evaluated still requires an offline batch pass over the archived SQLites — that remains deferred, and the snapshots it needs are exactly what the retention policy preserves. Nothing in the newsletter surfaces history yet. And scoring on decay is untouched by design.
---

## 2026-09-21 — token outage, list cleanup, and the classifier-instability fix

### What happened

The Max subscription ran out of tokens on another project, so the 09:00 UTC run had no LLM window. It exited 0 and published **247 domains with no toxic screening at all** — `Phase 2 ranker: FALLBACK — all_chunks_failed (44 chunks, none usable)`, `Snapshot classes: 0 legitimate, 0 parked, 0 toxic, 0 empty, 22 unknown`, `LLM backend: (none used)`.

**The alarm worked.** The subject line read `🚨 TOXIC SCREEN OFF / 🚨 PHASE 2 FALLBACK`. That is the 2026-09-18 escalation design doing exactly its job: the same failure went unnoticed for ~8 weeks after the July credit outage because a green SUCCESS verdict looked fine. This time it was visible the same morning.

The archive generator then chained on the successful exit and minted 2 permanent pages from the unscreened set.

### The cleanup

Three classification sweeps plus hand verification. **17 toxic domains removed**, denylist 6 → 23, published 247 → 230.

| pass | classified | toxic | published |
|---|---|---|---|
| 1 | 169 | 8 | 239 |
| 2 | 132 | 2 (+4 recorded by hand) | 234 |
| 3 | 113 | 3 (+1 by hand) | 230 |

Worst of them: `walgreenslistens.site` (Walgreens-survey brand-impersonation scam), `martyflynnforstatesenate.com` (Indonesian slot gambling on a squatted state-senate campaign domain), `veteransproject.net` (Chinese gambling platform), `giantbrandsolutions.com` (Chinese explicit adult), `catloversportal.com` / `shadesofamber.net` / `travelquestinternational.com` (casino portals in Chinese, Urdu-market English and Persian).

**Every verdict was hand-verified before being made permanent**, because a denylist entry is append-only and never aged out — a false positive blacklists a real domain forever. Two cases justified the care:

- `simplesolutionsbook2.com` — title reads "Simple Solutions – Mathematics Book". The body is a spam farm (slot x7, poker x4, betting x2, situs x1). **A clean title does not clear a domain**; only the body gave it away. Same shape as the 2026-09-19 safari business with injected lottery SEO.
- `triathlontraining.net` — the weakest call. No spam tokens, but repurposed into a gambling-adjacent content farm ("Online games", "How to Create a Responsible Gaming Checklist"), not a triathlon site. Recorded as a soft call; reversible.

**5 archive pages deleted** (`aee7dfb`), index 131 → 126, no orphans in either direction. Only `giantbrandsolutions.com` was minted today — `medicalbillingandcoding.net`, `nutramintsmartserum.com`, `shadesofamber.net` and `simplesolutionsbook2.com` were created on **earlier days**, so this was not purely today's fallout: pages were being minted for toxic domains throughout the period the screen was degraded.

### Why the classifier was unstable — the root cause

The dry run found **9** toxic; the real run 20 minutes later found **8**, overlapping by only **5**. `triathlontraining.net` was `toxic` in one pass and `legitimate` in the next.

`snapshot_classifier._fetch_excerpt_for` called archive.org on **every run for every domain and never consulted the sidecar**. So the model judged different evidence each time. The sidecar — 598 entries, 214 of them with real content — was written every run and **never read back**, meaning all that evidence was discarded and re-requested daily. That re-requesting is itself a large share of the archive.org load that makes the fetches fail (184 of 289 failed on the 2026-09-19 sweep). Self-inflicted.

Because `unknown` means BOTH "not abusive" and "the fetch failed", unstable evidence silently downgrades a **screened** domain back to **unscreened** — the mechanism that gave `ridgemotorsports.net` a permanent page on 2026-09-20 and the reason `toxic_denylist.py` exists. The fix applies the same principle one layer earlier: **remember the evidence, not just the verdict.**

**Not fixed, and not pretended otherwise**: the `claude_code` backend shells out to `claude -p --model ... --output-format json` with no temperature or seed flag, so identical evidence can still yield a different verdict. Stable evidence plus the existing skip-already-classified behaviour makes that academic in practice.

### What shipped (`ae0e9b3`)

1. **Excerpt reuse.** `classify_all` takes `excerpt_cache`; a usable cached excerpt is reused and the network call skipped; a cached `None` still **retries**, because retrying failures is how coverage improves. The load-bearing invariant, tested on both fetch paths: **a failed re-fetch can never replace a usable cached excerpt.** Reused excerpts do not sleep, which is most of the speed win. Both `pipeline.py` and `classify_carryover.py` feed the sidecar in and merge protectively on the way out — the previous `{**existing, **updates}` would have nulled the 214 content-bearing entries the moment reuse depended on them.

2. **Deterministic parked detection.** A parked page has no title, meta or headings, so it yields no signals and lands as `unknown`. `electricianyorkpa.com` (2,752 bytes, `data-adblockkey`, no title) became the site's **first ever `Clean` verdict at score 87** and got a permanent page. Markers now set `parked` with **no model call**, from the same single snapshot GET so archive.org sees no extra requests. Verified live: 6 domains converted on the first run.

3. **`scripts/spam_signatures.py`** — a deterministic second net beside the model. `attorneysportorange.com` was classified **`legitimate`** by the LLM while serving `9001cc金沙` casino-brand SEO dressed as an Inner Mongolia machinery company; a substring scan caught it instantly. Cloaking is what a model reading rendered prose is worst at and what a fingerprint match is best at. It also works when the LLM is down — which today proved is not hypothetical.

### Two false positives that are now permanent regression tests

- **`especialistas` contains `cialis`** — flagged a legitimate Spanish online psychology practice.
- **"appointment slots"** — flagged a Marrakech skincare clinic.

So `slot`, `bet`, `sex`, `poker`, `betting`, `lottery` and bare `cialis` are **excluded** from the term list, with the reasoning recorded in config so nobody re-adds them. Latin terms match on ASCII-alnum lookarounds rather than `\b`, because **Python's `\b` treats CJK ideographs as word characters** and would miss `金沙casino娱乐城` — the exact cloaked shape this net exists for.

### A config mistake caught by measurement before shipping

`excerpt_max_age_days` was briefly set to **90**, reasoning "don't judge on a stale capture". Measured against the real sidecar: only **12 of 249** usable excerpts fall inside 90 days, so 237 would still be re-fetched and the fix would have delivered about **5%** of its benefit. The error was conceptual — these are **dropped** domains, so the capture is old by definition and its age says nothing about accuracy. Now **0** (reuse indefinitely). Any future staleness rule should age against when *we* fetched, not when archive.org captured.

### Where the list ended up

| | morning | end of day |
|---|---|---|
| published | 247 | **230** |
| `unknown` (unscreened) | 171 | **93** |
| screened | 76 (31%) | **137 (60%)** |
| denylisted still published | — | **NONE** |
| archive pages for toxic domains | 5 | **NONE** |

Denylist 6 → 23. Sidecar 598 entries, 252 usable and now protected from being overwritten by a failed fetch. **1855 tests passing** (1395 at the start of the day). `CLASSIFIER_VERSION` v2 → v3.

### The 93 still unknown

Triaged individually rather than left at "we don't know". Of the 53 that could be retrieved: **10 parked, 31 empty captures, 11 ordinary small businesses, 0 abusive**. The single keyword hit was the Marrakech beauty clinic. The other ~40 have no retrievable snapshot at all and cannot be certified either way — but nothing in the examined half suggests abuse is hiding there.

A deterministic scan of all **stored** excerpts against the published list now returns exactly zero spam signatures, so everything we have content for is clean.

### Open

- **Archive pages are minted from `Clean`/`Promising` verdicts that can be produced by ABSENCE of data.** `electricianyorkpa.com` scored 87 and was the only Clean in 247, purely because `open_page_rank` and `cc_source_domain_count` were both null and nulls are excluded from the scoring average — every fully-enriched domain topped out at 61. So the verdict rewards missing signals. Parked detection removes this particular instance, but the scoring asymmetry is untouched and is the more important issue. STATE's earlier note that Clean is "effectively unreachable at ≥70" was wrong in an unhelpful direction: it is reachable, but mainly by knowing less.
- The `OnSuccess=` chain mints archive pages from a run whose screen did not run. A green exit does not mean the domains were screened; consider gating page generation on the screen having actually produced verdicts.
- Model-level nondeterminism remains (no temperature/seed on the subscription backend).
---

## 2026-09-22 — two days of zero new domains: OpenPageRank migrated, nobody noticed

### It was never the filters

The run was clean and the funnel healthy: 177,290 drops → 157,123 structural → 50,394 lexical → 4,830 above the ranker gate (37.2%) → 2,664 bucketed for RDAP → **25 confirmed available** → 17 past the post-enrichment filter. The ranker ran perfectly (44/44 chunks, 0 failed).

All 17 then died at the **publish quality floor**:

```
scripts.output Quality floor: kept 216, rejected 5 (score) + 12 (completeness) of 233 input
```

### Root cause

**OpenPageRank moved from DomCop to Keywords Everywhere and the old API was switched off, not deprecated.** `openpagerank.com` now 301-redirects to `openpagerank.keywordseverywhere.com`, which 404s the old path. Because `requests` follows redirects by default, the log showed a bare *"404 Not Found"* against a URL nobody had configured — so it read like a transient outage rather than a migration, the circuit breaker opened after 5 failures exactly as designed, and the run continued quietly.

The publish gate needs **3 of 5** enrichment fields. Measured on today's 25 available candidates:

| field | present |
|---|---|
| wayback_snapshots | 25/25 |
| wayback_last_snapshot | 25/25 |
| open_page_rank | **0/25** (API migrated) |
| cert_history | **0/25** (crt.sh 502s, most days) |
| previous_registrar | **0/25** (structural) |

2/5 = 0.40 against a 0.50 threshold → every new domain rejected. **With OPR alive: 3/5 = 0.60 and all 25 would have published.**

### The structural fragility this exposed

`previous_registrar` is present in **0 of 216** published rows. It cannot be populated for an AVAILABLE domain, and an available domain is the only kind we publish. So the gate scores **4 obtainable fields against a 5-field denominator**, and since `cert_history` depends on crt.sh — which 502s most days — **OpenPageRank was silently load-bearing for the entire publish step.** One third-party migration zeroed the output.

Timeline, which makes the causality unambiguous:

| date | new published | completeness rejections |
|---|---|---|
| 09-18 | 45 | 0 |
| 09-19 | 79 | 0 |
| 09-20 | 40 | 0 |
...
2026-09-21's zero was masked by the token-outage headline; it had the same cause.

Per the owner's decision the gate itself was **left unchanged** — the right call under pressure, since changing what qualifies for publication while diagnosing an outage conflates two decisions. It should be revisited once OPR is healthy and can be judged on its merits.

### The migration

```
old (dead): GET  openpagerank.com/api/v1.0/getPageRank?domains[]=x
            header API-OPR: <key>
            body   {"response":[{"page_rank_decimal": 3.7}]}

new:        POST openpagerank.keywordseverywhere.com/v1/domains/bulk
            header Authorization: Bearer opr_live_...
            send   {"domains":["x"], "include_history": false}
            body   {"as_of":..., "results":[{"domain":..., "found":true,
                                             "open_page_rank":9.54, "rank":21,
                                             "referring_domains":...}]}
```

The emitted field is unchanged, so the output contract (PLAN.md Principle 5) is untouched.

**`found: false` returns `{"open_page_rank": 0.0}`, NOT `{}`.** This mirrors the old API's `page_rank_decimal: 0` and is load-bearing: an empty dict leaves the field ABSENT and costs a point of completeness at the publish gate, which would quietly reject exactly the obscure dropped domains this project exists to surface. A malformed payload still returns `{}` — failing to measure is not a measurement of zero. Verified live: of 12 real candidates, 9 returned 0.0 and 3 returned real scores (0.09, 0.11, 0.79); all 12 got the field.

**Credentials changed.** Legacy DomCop keys are rejected with `401 {"type":"authentication_error","message":"Invalid OPR API key."}`, confirmed against the live API with the production key. KE keys and OPR keys are **distinct**: the Keywords Everywhere key is what you sign in to the dashboard with, and the OPR key (`opr_live_…`) is minted there separately. A new key was minted and written to `.env` (mode 0600, ownership preserved). `401/403` now logs at ERROR naming the legacy-key cause, because a rejected key silently zeroes the publish gate and no retry fixes it.

**Free tier is 30,000 domains/month** against our ~2,400. No spend.

### The second defect: the new API is much slower

Restoring the endpoint alone would have left the pipeline still broken. Measured on the box over 10 single-domain calls:

```
min 2.56s   median 6.57s   max 29.91s   -- 3 of 10 exceeded the global 10s timeout
```

At roughly a third of calls timing out, and 5 consecutive failures opening the breaker for 15 minutes, OPR would have gone dark mid-run anyway — the pipeline would have *looked* fixed and still published nothing. Added `api_timeout_seconds`, a per-source override falling back to the global, mirroring the existing `api_min_interval_seconds` dict. OPR is 45s. No other source changes behaviour.

A later verification run saw one call at 52s (requests' `timeout` is per-read, not total), and still returned a value. Latency is now the main operational cost of this source: ~8s median x 25–80 candidates.

**Deferred, deliberately:** the endpoint accepts **100 domains per request**. Batching is the real answer to this latency — one call instead of 25–80 slow serial ones — but it needs the per-domain plugin contract (hard rule 8) revisited, so it is recorded in config as a follow-up rather than smuggled into an outage fix.

### Verified end to end

12 of 12 of today's rejected candidates now return `open_page_rank`, taking each from 2/5 to 3/5 completeness. 1870 tests passing. OPR tests rewritten for the new API (8 → 23), covering `found:false`, the exact 404 that caused this, both auth-rejection codes, the per-source timeout, and a guard against reverting to the dead DomCop URL. Fixtures now use invented domains — the previous file used `example.com`, against hard rule 1.

### Cost of the outage

Two days of finds are gone: 17 candidates on 09-21 and 25 on 09-22, all confirmed available and rejected solely for a missing third-party field. They are not in carryover because they never published, and the daily diff is day-over-day so tomorrow's run will not rediscover them. Today's 25 survive with their enrichment in `scripts/state/available_set_latest.jsonl` and in the permanent domain archive, so a targeted re-enrich-and-publish is possible if wanted.

### Lesson worth keeping

A dead upstream dependency presented as a 404 inside a fail-soft enricher, and fail-soft is correct — but it meant a *silent* loss of the single field holding the publish gate up. The daily report shows enrichment completeness nowhere. **Consider surfacing per-source enrichment success rates in the daily email**, the same way CC staleness and the LLM backend already are; a line reading `open_page_rank: 0/25` on 09-21 would have cost one minute instead of two days.
---

## 2026-09-23 — making every step declare whether it actually did anything

### The pattern, finally named

Every incident in this project has one shape: **a step fails soft, returns empty, the next step treats empty as a legitimate value, and nobody learns until output visibly drops.**

| incident | silent for |
|---|---|
| LLM credits died | **8 weeks** |
| Common Crawl went stale | **4 months** |
| OpenPageRank migrated | **2 days of zero new domains** |
| crt.sh 502s (~12 failures in the 09-23 run alone) | ongoing, untracked |

Fail-soft is correct for uptime. It was implemented as fail-**silent**, and that is what hollows out the meaning of each step: they still run, they just stop contributing, and nothing says so.

### 1. Every step reports its coverage

`pipeline.py` emits one contract line after enrichment:

```
Enrichment coverage (25 candidates): wayback_snapshots=25/25 open_page_rank=0/25 cert_history=0/25 ...
```

A field that produced nothing still appears as `0/N` — that is the entire point; a field that silently drops out of the line teaches nothing. Baseline fields are pinned, extras are discovered from what the enrichers actually returned (so a newly wired source appears without editing a tuple), and the line is emitted even for an empty candidate set, so its **absence always means the stage did not run**.

`send_report.py` parses it, lists per-source coverage, marks anything below `warn_below_ratio` as degraded, and escalates the subject to **`🚨 SOURCE DEAD (open_page_rank)`** when a source in `alarm_sources` returns nothing. Naming the source in the subject is deliberate — that is the one-minute fix. A *missing* coverage line is itself reported (without escalating), because silence is the failure mode this exists to eliminate.

`alarm_sources` is deliberately only `wayback_snapshots` and `open_page_rank`: crt.sh 502s in bursts most days and would cry wolf, and `cc_source_domain_count` is legitimately absent for domains outside the CC graph.

Verified on the box against the real config: healthy → no escalation; `open_page_rank=0/25` → escalates; missing line → no false alarm.

### 2. Gates only measure obtainable things

The publish completeness gate counted five FIELDS, two of them wrongly:

- **`previous_registrar` is present in 0 of 216 published rows and always will be** — RDAP returns not-found for an AVAILABLE domain, and available is the only kind we publish. The gate scored against something impossible by construction.
- **`wayback_snapshots` + `wayback_last_snapshot` are ONE source counted twice**, letting Wayback alone carry 40% of the gate.

Together they are why one third-party API migration produced two days of zero published domains: candidates scored 2/5 = 0.40 against a 0.50 threshold while the filters behaved perfectly normally.

Now counts distinct **sources** with at least one signal. The 0.50 threshold is unchanged and means 2 of 3. Measured on the live 216 rows: **184 at 1.00, 32 at 0.67, zero below the gate.** A missing or malformed config key falls back to the old five-field counting rather than crashing.

**Deliberately NOT outage-proof.** Lose OPR again and candidates fall to 1 of 3 and publishing stops — which is correct, because we genuinely would not know enough about them. The defect was never that we published nothing; it was that nobody knew why for two days. Change 1 is what closes that.

### 3. Missing data cannot buy a verdict

Scoring excludes nulls from the weighted average and renormalises the rest, so **a domain we know less about can outscore one we know more about.** Live, not theoretical: the two-source rows top out at **87** while the three-source rows top out at **61**.

`electricianyorkpa.com` scored 87 — the highest row on the site, the only `Clean` ever published — precisely because `open_page_rank` and `cert_history` were missing. Archive pages are minted from `Clean`/`Promising`, so the inversion propagated into permanent indexed content.

`Clean` now requires **3** real sources, `Promising` **2**, counted the same way as the gate. **The score maths is deliberately untouched** (owner's call — changing it re-ranks everything and needs recalibration against real runs); the floor is an additional necessary condition that can only demote, never promote. Both thresholds default to 0 = disabled, so an old config verdicts exactly as before.

Measured on the live 216 rows: **exactly one verdict changes** — `electricianyorkpa.com` Clean → **Caution**. It fails the Clean floor at 2 of 3 sources, then fails the Promising re-test on `wayback 578 < 1000`. Archive-page eligibility 10 → 9, and that page is the one that should not have been minted.

One deliberate asymmetry: the verdict floor does **not** credit a `wayback_unknown` candidate with the wayback source, though the publish gate still does (the 2026-05-17 exemption). A failed fetch and "a source that returned a value" cannot both be true when awarding a top-tier badge. Affects 2 of 216 rows and only ever demotes.

`verdict` is a **behaviour** change, not a schema change — same field, same three strings. Documented in PLAN.md Principle 5 and `output.py`, including the warning that consumers recomputing a verdict from `score` alone (the frontend/newsletter fallback for sample data) do **not** inherit the floor.

### Also today

The 09:00 run confirmed the OpenPageRank fix: **completeness rejections 12 → 12 → 0**, and 14 new domains published (`first_seen_date: 2026-09-23`).

**1967 tests passing** (+99).

### Open

- **`src/content/archive/electricianyorkpa.com.md` still asserts a `Clean` verdict** we have now retracted, and its body reasons from absence — *"The absence of any spam or malware signals in available sources supports the Clean classification"* — which is exactly the inference this change rejects. The archive generator's idempotency guard will not regenerate it. It should be deleted or regenerated deliberately.
- **crt.sh is the next weak source**: ~12 failures in today's run, one taking 65 seconds, and `cert_history` sits at ~182/216. It is now visible in the daily report rather than invisible, which is the precondition for deciding what to do about it.
- The scoring inversion is contained, not removed — a sparse row can still out-*score* a rich one, it just cannot out-*rank* it into a verdict tier.
